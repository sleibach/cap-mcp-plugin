...
class McpPlugin {
    ...
    onModelLoaded(model) {
        ...
        if (explicitKind === "memory" || explicitKind === "stateless")
            return;
    ...
    async onLoaded(model) {
        ...
        // Seed the session store with runtime config + annotations so that
        // stores which rehydrate sessions (e.g. DB store) can reconstruct an
        // identical MCP server on ANY instance — not just the one that
        // originally handled `initialize`.
        this.sessionManager?.setRuntimeConfig?.(this.config, this.annotations);
    ...
    registerMcpSessionRoute() {
        ...
        this.expressApp?.post("/mcp", async (req, res) => {
            ...
            // Stateless mode: build a fresh transport per request, handle, then
            // close. No session IDs are issued or tracked, so session-id header
            // and isInitialize branching are irrelevant.
            if (this.sessionManager.isStateless) {
                const session = await this.sessionManager.createSession(this.config, this.annotations);
                try {
                    const t0 = Date.now();
                    await session.transport.handleRequest(req, res, req.body);
                    logger_1.LOGGER.debug("MCP stateless request handled", { durationMs: Date.now() - t0 });
                }
                catch (e) {
                    if (!res.headersSent) {
                        res.status(500).json({
                            jsonrpc: "2.0",
                            error: { code: -32603, message: "Internal Error: Transport failed", id: null },
                        });
                    }
                }
                finally {
            ...
            if (!session) {
                // Per MCP streamable-HTTP spec: an unknown/expired session ID must
                // return 404 (not 400). That is the signal clients use to know they
                // must reinitialize. A 400 is treated as a generic error and the
                // client stalls indefinitely — the exact "hung after idle" symptom.
                // Also include WWW-Authenticate so that if the token has also expired
                // the client discovers the auth server in the same round-trip.
                logger_1.LOGGER.warn("Session not found or expired — client must reinitialize", { sessionId: sessionIdHeader });
                if (this.config.auth !== "none") {
                    try {
                        res.setHeader("WWW-Authenticate", (0, utils_2.wwwAuthenticateHeader)(req, this.config));
                    }
                    catch { /* best effort — never block the 404 on header build failure */ }
                }
                res.status(404).json({
                    jsonrpc: "2.0",
                    error: {
