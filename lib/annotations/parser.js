...
function constructResourceAnnotation(serviceName, target, annotations, definition, model) {
    ...
    const omittedFields = new Set(Object.entries(model.definitions?.[entityTarget].elements ?? {})
        ...
        .map(([k, _]) => k));
    // Per-path omission map: { navProp: Set<omittedField> } — used by the
    // response-shaping step to strip fields from expanded subtrees as well.
    // Pre-built here so we don't walk the CSN at every response.
    const elements = model.definitions?.[entityTarget]?.elements ?? {};
    const navigations = new Map();
    for (const [elKey, el] of Object.entries(elements)) {
        if (el.type === "cds.Composition") {
            navigations.set(elKey, { kind: "Composition", target: el.target });
        }
        else if (el.type === "cds.Association") {
            navigations.set(elKey, { kind: "Association", target: el.target });
        }
    }
    logger_1.LOGGER.debug("Parsed resource annotation", { entityTarget, omittedFields: Array.from(omittedFields), navigations: Array.from(navigations.keys()) });
    const { properties, resourceKeys, propertyHints } = (0, utils_1.parseResourceElements)(definition, model);
    // Pull in any service-level `@requires` / `@restrict` so per-session tool
    // filtering honours service-wide gates (e.g. `@requires:'admin' service
    // AdminService { ... }`). CAP enforces these at execution time but does
    // not copy them onto the entity definition, so without this merge the
    // MCP tool-visibility gate would see `restrictions: []` for every entity
    // in a gated service and leak admin tools to non-admin callers.
    const serviceDef = model.definitions?.[serviceName];
    const entityReq = annotations.requires ?? definition["@requires"];
    const entityRestrict = annotations.restrict ?? definition["@restrict"];
    const serviceReq = serviceDef?.["@requires"];
    const serviceRestrict = serviceDef?.["@restrict"];
    const effectiveRequires = entityReq ?? serviceReq;
    const effectiveRestrict = entityRestrict ?? serviceRestrict;
    const restrictions = (0, utils_1.parseCdsRestrictions)(effectiveRestrict, effectiveRequires);
    // @mcp.expand (entity-level) — 'compositions' | 'none' | 'all' | undefined
    const expandMode = typeof annotations.expand === "string" && ["compositions", "none", "all"].includes(annotations.expand)
        ? annotations.expand
        : undefined;
    return new structures_1.McpResourceAnnotation(annotations.name, annotations.description, target, serviceName, functionalities, properties, resourceKeys, foreignKeys, annotations.wrap, restrictions, computedFields, propertyHints, omittedFields, expandMode, navigations);
}
/**
...
function constructToolAnnotation(model, serviceName, target, annotations, entityKey, keyParams) {
    if (!(0, utils_1.isValidToolAnnotation)(annotations))
        return undefined;
    const { parameters, operationKind, propertyHints, optionalParams } = (0, utils_1.parseOperationElements)(annotations, model);
    // Fall back to service-level `@requires` / `@restrict` like constructResourceAnnotation does —
    // otherwise actions/functions in a role-gated service carry no restrictions at all and their
    // tools stay visible to callers the service itself would reject.
    const serviceDef = model.definitions?.[serviceName];
    const effectiveRequires = annotations.requires ?? serviceDef?.["@requires"];
    const effectiveRestrict = annotations.restrict ?? serviceDef?.["@restrict"];
    const restrictions = (0, utils_1.parseCdsRestrictions)(effectiveRestrict, effectiveRequires);
    return new structures_1.McpToolAnnotation(annotations.name, annotations.description, target, serviceName, parameters, entityKey, operationKind, keyParams, restrictions, annotations.elicit, propertyHints, optionalParams);
}
/**
