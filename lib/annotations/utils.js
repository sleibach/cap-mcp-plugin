...
function parseResourceElements(definition, model) {
    ...
    for (const [k, v] of Object.entries(definition.elements || {})) {
        ...
        // Inline / named structured elements without a resolved `.type` or
        // `.ref` cannot be mapped to a Zod leaf. CAP flattens structured
        // persistence elements to `parent_child` at the service layer, so the
        // flat siblings are iterated alongside this entry — skipping the
        // parent here avoids a crash inside `parseTypedReference` while still
        // exposing every scalar the entity actually carries.
        if (typeof v.type !== "string" && !v.type?.ref) {
            continue;
        }
...
function parseOperationElements(annotations, model) {
    ...
    if (params && Object.entries(params).length > 0) {
        ...
        for (const [k, v] of Object.entries(params)) {
            // A parameter is required only when explicitly marked; otherwise expose it as optional so
            // agents are not forced to supply every parameter (e.g. "at least one of" actions).
            // NOTE: `key` is intentionally not treated as mandatory here — operation params typed as a
            // reference to a key field (e.g. `foo: SomeEntity:ID`, plain type reuse) inherit `key: true`
            // after linking even though they are not mandatory. Genuine bound-operation keys are handled
            // separately via keyTypeMap, not here.
            const isMandatory = v["@mandatory"] === true || v.notNull === true;
            if (!isMandatory) {
...
function parseCdsRestrictions(restrictions, requires) {
    ...
    const result = [];
    if (requires) {
        // `@requires` accepts a single role or an array of alternative roles. Fan arrays out into
        // one entry per role — consumers match entries via `user.is(entry.role)`, which expects a
        // scalar role name and would never match an array (hiding the tool for every caller, even
        // ones that hold all of the listed roles, e.g. via an AMS Admin policy).
        for (const role of Array.isArray(requires) ? requires : [requires]) {
            result.push({ role });
        }
    }
