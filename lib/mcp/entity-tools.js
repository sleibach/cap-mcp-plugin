...
function classifyCapError(error, defaultCode, toolName) {
    ...
    if (capCode === "DRAFT_LOCKED_BY_ANOTHER_USER" ||
        ...
        return { code: "DRAFT_LOCKED", msg: `${toolName}: draft is locked by another user` };
    }
...
function draftRequiredError(resAnno, op) {
    ...
    const steer = op === "delete"
        ? `Use ${discard} to drop an existing draft, or ${edit} + ${activate} if you need to activate a pending draft first.`
        : `Use ${edit} to create a draft, ${patch} to apply changes, then ${activate} to publish.`;
...
function registerEntityWrappers(resAnno, server, authEnabled, defaultModes, accesses) {
    ...
    // Auto-enable draft lifecycle tools for `@odata.draft.enabled` roots. Users
    // can opt out by listing explicit modes on `@mcp.wrap.modes` without the
    // draft-* entries.
    const baseModes = explicitModes ?? defaultModes;
    const modes = explicitModes
        ? baseModes
        : draftEnabled
            ? [...baseModes, "draft-new", "draft-edit", "draft-patch", "draft-activate", "draft-discard"]
            : baseModes;
...
function registerUpdateTool(resAnno, server, authEnabled) {
    ...
    const updateHandler = async (args) => {
        ...
        // draft-* tool instead.
        if (isDraftEnabledRoot(resolveEntityDefinition(resAnno))) {
...
function registerDeleteTool(resAnno, server, authEnabled) {
    ...
    const deleteHandler = async (args) => {
        ...
        // workflow is in force). Emit DRAFT_REQUIRED.
        if (isDraftEnabledRoot(resolveEntityDefinition(resAnno))) {
