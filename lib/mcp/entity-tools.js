...
function expandAssociationKeys(def, propName) {
    ...
    if (keys.length === 0) {
        ...
        return [{ fkColumn: `${propName}_ID`, targetKey: "ID", cdsType: "String", isMandatory: false }];
    }
    return keys.map((k) => {
        const targetKey = Array.isArray(k?.ref) ? k.ref[0] : k?.ref;
        const targetElement = targetDef?.elements?.[targetKey];
        const cdsType = typeof targetElement?.type === "string"
            ? targetElement.type.replace(/^cds\./, "")
            : "String";
        const isMandatory = Boolean(assoc?.["@mandatory"] || targetElement?.["@mandatory"]);
        return { fkColumn: `${propName}_${targetKey}`, targetKey, cdsType, isMandatory };
    });
}
/**
...
function classifyCapError(error, defaultCode, toolName) {
    ...
    if (capCode === "DRAFT_LOCKED_BY_ANOTHER_USER" ||
        ...
        return {
            code: "DRAFT_LOCKED",
            msg: `${toolName}: draft is locked by another user${holderHint}. Retry as that user or wait for cds.env.drafts.cancellationTimeout (default 15 min), then ${toolName.replace(/_[^_]+$/, "_draft-discard")} to release.`,
        };
    }
    ...
    // CSN" failure. The EAM-class bug (missing FK columns on composite-key
    // associations) surfaces here. Promote it so the LLM sees the intent.
    if (capCode === "ASSERT_DATA_TYPE" || rawMsg.includes("ASSERT_DATA_TYPE")) {
        return {
            code: "ASSERT_DATA_TYPE",
            msg: `${toolName}: ${rawMsg}. Payload shape did not match CSN — verify every composite-key FK column (e.g. \`assoc_Key1\` + \`assoc_Key2\` for multi-key associations) and every structured element is supplied.`,
        };
    }
    ...
    const isDraftOp = /_draft-(new|edit|patch|activate|discard)$/.test(toolName || "");
    ...
    if (isAssertCode ||
        ...
        if (isDraftOp) {
            ...
            return {
                code: onActivate ? "DRAFT_ACTIVATE_FAILED" : "DRAFT_VALIDATION_FAILED",
                msg: `${toolName}: ${rawMsg}${fieldHint}. ${onActivate
                    ? "Fill missing @mandatory / @assert.* fields via draft-patch before activating."
                    : "Adjust field values and retry the draft-patch/new call."}`,
            };
        }
    }
    if (capCode === 404)
...
function registerEntityWrappers(resAnno, server, authEnabled, defaultModes, accesses) {
    ...
    const DRAFT_MODES = ["draft-new", "draft-edit", "draft-patch", "draft-activate", "draft-discard"];
...
function registerCreateTool(resAnno, server, authEnabled) {
    ...
    const createHandler = async (args) => {
        ...
        // every generated column (not just `_ID`).
        const def = resolveEntityDefinition(resAnno);
        const data = {};
        for (const [propName, cdsType] of resAnno.properties.entries()) {
            ...
            if (isAssociation) {
                for (const fk of expandAssociationKeys(def, propName)) {
                    if (args[fk.fkColumn] !== undefined) {
                        data[fk.fkColumn] = coerceNumeric(args[fk.fkColumn]);
                    }
                }
                continue;
            }
...
function registerUpdateTool(resAnno, server, authEnabled) {
    ...
    const updateHandler = async (args) => {
        ...
        // generated FK columns and coerce numeric strings.
        const updates = {};
        ...
        for (const [propName, cdsType] of resAnno.properties.entries()) {
            ...
            if (isAssociation) {
                for (const fk of expandAssociationKeys(defForUpdate, propName)) {
                    if (args[fk.fkColumn] !== undefined) {
                        updates[fk.fkColumn] = coerceNumeric(args[fk.fkColumn]);
                    }
                }
                continue;
            }
...
function logDraftOp(op, resAnno, keysOrData) {
    try {
        const CDS = global.cds;
        const user = CDS?.context?.user?.id || "unknown";
...
function registerDraftNewTool(resAnno, server, authEnabled) {
    ...
    const handler = async (args) => {
        ...
        try {
            ...
            const response = await withTimeout(svc.tx({ user: (0, utils_1.getAccessRights)(authEnabled) }, (tx) => tx.send("NEW", `${resAnno.target}.drafts`, data)), TIMEOUT_MS, toolName);
            const merged = { ...data, ...(response && typeof response === "object" ? response : {}) };
