...
exports.registerReadBundleTool = registerReadBundleTool;
exports.groupKeyFor = groupKeyFor;
exports.buildReadBundleDescription = buildReadBundleDescription;
exports.bundleToolName = bundleToolName;
exports.resourceIdFor = resourceIdFor;
exports.toolNameFor = toolNameFor;
exports.withTimeout = withTimeout;
exports.classifyCapError = classifyCapError;
...
// Elements CAP's draft runtime manages itself — stamping IsActiveEntity,
// maintaining the SiblingEntity link, tracking the DraftAdministrativeData
// header. Exposing these as writable parameters tempts the LLM to supply
// bookkeeping values the user never asked for, and CAP will either ignore
// them or error. Applied to writable input schemas (create / update /
// draft-new / draft-patch); read-side schemas still need IsActiveEntity
// so callers can pick the draft vs. the active sibling.
const DRAFT_INTERNAL_FIELDS = new Set([
    "IsActiveEntity",
    "HasActiveEntity",
    "HasDraftEntity",
    "SiblingEntity",
...
/**
 * Composes the `.describe(...)` text for a writable field on create /
 * draft-new / draft-upsert input schemas. Three cases matter to the LLM:
 * 1. UUID keys (CAP's `cuid` aspect). CSN does not set `@cds.on.insert`
 *    on these, but the runtime still auto-generates the UUID on insert.
 *    The caller MAY supply one for idempotency or external correlation;
 *    otherwise the server generates and returns it. Make both paths explicit.
 * 2. Foreign keys — note the referenced entity so the LLM knows what to
 *    look up before attempting to write.
 * 3. Everything else — fall through to the property hint parsed from
 *    `@mcp.hint` / `@description` annotations.
 */
function describeWritableField(resAnno, propName, cdsType) {
    const hint = resAnno.propertyHints?.get(propName) ?? "";
    const isKey = resAnno.resourceKeys?.has?.(propName);
    const typeStr = String(cdsType ?? "").toUpperCase();
    const isUuid = typeStr === "CDS.UUID" || typeStr === "UUID";
    if (isKey && isUuid) {
        return `Key ${propName} (UUID). Optional — CAP generates a UUID on insert when omitted and returns it in the response. Supply your own only to pin a specific ID (e.g. idempotency, external correlation). ${hint}`.trim();
    }
    if (resAnno.foreignKeys?.has?.(propName)) {
        return `Foreign key to ${resAnno.foreignKeys.get(propName)} on ${propName}. ${hint}`.trim();
    }
    return `Field ${propName}. ${hint}`.trim();
}
/**
...
function expandAssociationKeys(def, propName) {
    ...
    if (keys.length === 0) {
        ...
        // to-many associations (cardinality.max === '*') have no FK columns by definition — they
        // are navigation back-references, not scalar FK fields. Return [] silently so the calling
        // schema-builder simply skips them without emitting a bogus <name>_ID parameter.
        if (assoc?.cardinality?.max === "*") {
            return [];
        }
        // Framework-internal associations added by CAP (changelog, localized, etc.) have no type
        // string in the CSN. Skip them silently rather than emitting noise on every tool call.
...
/**
 * Extracts the structured fields CAP errors carry so MCP clients get the
 * same detail an OData client would receive (raw `code`, `target`, nested
 * `details[]`, HTTP status, template args, numeric severity). Every value
 * is defensively stringified/copied so the payload is JSON-safe.
 */
function extractCapErrorDetails(error) {
    if (!error || typeof error !== "object") return undefined;
    const out = {};
    if (error.code !== undefined && error.code !== null) out.capCode = error.code;
    const status = error.status ?? error.statusCode;
    if (typeof error.code === "number") out.httpStatus = error.code;
    else if (typeof status === "number") out.httpStatus = status;
    if (error.target) out.target = String(error.target);
    if (error.element) out.element = String(error.element);
    if (Array.isArray(error.args) && error.args.length) out.args = error.args.map(String);
    const severity = error["@Common.numericSeverity"] ?? error.numericSeverity;
    if (typeof severity === "number") out.numericSeverity = severity;
    if (Array.isArray(error.details) && error.details.length) {
        out.details = error.details.map((d) => ({
            code: d?.code,
            message: d?.message,
            target: d?.target,
            element: d?.element,
            numericSeverity: d?.["@Common.numericSeverity"] ?? d?.numericSeverity,
            details: Array.isArray(d?.details) && d.details.length
                ? d.details.map((n) => ({ code: n?.code, message: n?.message, target: n?.target }))
                : undefined,
        })).map((d) => {
            // drop undefined keys so the wire payload stays tidy
            const clean = {};
 ...
 * structured code it can reason about. Returns an `extra` object with the
 * raw CAP fields (capCode, httpStatus, target, details[]) so MCP clients
 * can reason about the same surface OData clients see. Recognized:
...
function classifyCapError(error, defaultCode, toolName) {
    ...
    const extra = extractCapErrorDetails(error);
    if (rawMsg.includes("timed out")) {
        return { code: "TIMEOUT", msg: `${toolName} timed out after ${TIMEOUT_MS}ms`, extra };
    }
    ...
    if (capCode === "DRAFT_LOCKED_BY_ANOTHER_USER" ||
        ...
        const diagnostic = holder && caller && holder !== caller
            ? " Root cause: the lock holder and the current MCP principal are different identities. If you expected the same human to drive both calls, check whether the MCP bearer token's `sub` / `user_name` claim matches the holder, and whether CAP's auth middleware runs on `/mcp` (enable DEBUG=mcp to trace)."
            : "";
        return {
            code: "DRAFT_LOCKED",
    ...
    if (capCode === "DRAFT_ALREADY_EXISTS" || rawMsg.includes("DRAFT_ALREADY_EXISTS")) {
        return { code: "DRAFT_ALREADY_EXISTS", msg: `${toolName}: draft already exists for this row`, extra };
    }
    ...
    if (capCode === 404)
        return { code: "NOT_FOUND", msg: `${toolName}: ${rawMsg}`, extra };
    if (capCode === 409 || capCode === 412)
        return { code: "CONFLICT", msg: `${toolName}: ${rawMsg}`, extra };
    // 401/403 must never be masked — clients need to know when the failure is
    // authorization, not a runtime bug. CAP's generic auth rejection often
    // throws with a bare `new Error()` carrying only `code: 403`, so `rawMsg`
    // is just "Error". Surface the HTTP status explicitly and offer the
    // principal/role context the caller needs to debug.
    if (capCode === 403) {
        const CDS = global.cds;
        const caller = CDS?.context?.user?.id ?? "<unknown>";
        const roles = CDS?.context?.user?.roles;
        const rolesHint = roles
            ? ` (roles: ${Array.isArray(roles) ? (roles.join(", ") || "none") : Object.keys(roles).join(", ") || "none"})`
            : "";
        return {
            code: "FORBIDDEN",
            msg: `${toolName}: access denied (HTTP 403). The authenticated user '${caller}'${rolesHint} lacks permission for this operation — check the @requires / @restrict annotations on the service or entity.${rawMsg && rawMsg !== "Error" ? ` Detail: ${rawMsg}` : ""}`,
            extra,
        };
    }
    if (capCode === 401) {
        return {
            code: "UNAUTHORIZED",
            msg: `${toolName}: not authenticated (HTTP 401). The request arrived without a valid identity — check that the MCP client sent credentials (Authorization header / bearer token) and that the CAP auth middleware ran before the tool handler.${rawMsg && rawMsg !== "Error" ? ` Detail: ${rawMsg}` : ""}`,
            extra,
        };
    }
    return { code: defaultCode, msg: `${defaultCode}: ${rawMsg}`, extra };
}
/**
...
function formatCapErrorMessage(error) {
    if (!error) return "";
    // CAP's generic auth reject often throws a bare `new Error()` whose
    // `.message` is an empty string or the literal "Error" from toString().
    // In that case fall back to the CAP `code` (numeric HTTP status or
    // symbolic code) so the message carries SOME signal, rather than leaving
    // the client with "QUERY_FAILED: Error".
    const rawMessage = error.message || String(error);
    const top = rawMessage && rawMessage !== "Error"
        ? rawMessage
        : (error.code != null ? `HTTP ${error.code}` : "Error");
    const details = Array.isArray(error.details) ? error.details : [];
 ...
 */
function registerEntityWrappers(resAnno, server, authEnabled, defaultModes, accesses, modesOverride) {
    const CDS = global.cds;
    ...
    //
    // `modesOverride`, when provided, takes precedence over the entity's own
    // `@mcp.wrap.modes` — used by the read-bundle path to register ONLY the
    // write/draft modes per-entity (query/get move into the consolidated read
    // tool). Draft auto-enable below still applies on top of the override.
    const baseModes = modesOverride != null ? modesOverride : (explicitModes ?? defaultModes);
    const DRAFT_MODES = ["draft-new", "draft-edit", "draft-patch", "draft-activate", "draft-discard", "draft-upsert"];
    // `@mcp.wrap { draft: false }` opts an entity out of the draft auto-enable —
    // for draft-enabled roots whose draft lifecycle should not be exposed as MCP
    // tools (e.g. write access goes through dedicated actions instead).
    const skipDraft = resAnno.wrap?.draft === false;
    const modes = (draftEnabled && !skipDraft)
    ...
    // On draft-enabled entities (without `@odata.draft.bypass` / the global
    // `cds.env.fiori.bypass_draft`) the plain create/update writers always
    // fail with DRAFT_REQUIRED at the CAP service layer — they can't bypass
    // the draft lifecycle. Exposing them as callable tools gives the agent
    // dead-end surface area that always errors. Skip registration on pure
    // draft-enabled roots; draft-new and draft-edit+patch already cover the
    // create / update intents. Bypass-enabled roots still register the
    // active-row tools since the server accepts direct mutations there.
    // DELETE on the active entity is allowed by CAP for draft-enabled roots,
    // so it stays registered unconditionally.
    const draftBypass = isDraftBypassEnabled(resolveEntityDefinition(resAnno));
    const suppressActiveWriters = draftEnabled && !draftBypass;
    if (modes.includes("create") && accesses.canCreate && !suppressActiveWriters) {
...
function nameFor(resAnno, suffix) {
    return toolNameFor({
        serviceName: resAnno.serviceName,
        target: resAnno.target,
        mcpName: resAnno.name,
    }, suffix);
}
/**
 * Shared tool-naming helper. Exported so `cap_describe_model` (and anything
 * else rendering example tool calls) produces names identical to what we
 * pass to `server.registerTool`. Takes the same three inputs as `nameFor`
 * without requiring a full `resAnno`, so reflection-only callers work too.
 * Keep `nameFor` as the internal convenience wrapper.
 */
function toolNameFor({ serviceName, target, mcpName }, suffix) {
    const entityName = String(target || "Entity").split(".").pop();
    const svcName = String(serviceName || "Service").split(".").pop();
    const legacy = `${svcName}_${entityName}_${suffix}`;
    const fromAnno = mcpName && typeof mcpName === "string"
        ? `${mcpName}_${suffix}`
        : null;
    /** @type {string[]} */
    const candidates = [];
    ...
    // The configured tool_name_prefix is added later (registerTool wrapper), so reserve its
    // length here to keep the FINAL prefixed name within the 64-char MCP limit.
    const prefixOverhead = (0, loader_1.applyToolNamePrefix)("").length; // 0 when no prefix
    const maxLen = 64 - prefixOverhead;
    const fits = candidates.find((n) => n.length <= maxLen);
    if (fits)
        return fits;
    const best = candidates.reduce((a, b) => (a.length <= b.length ? a : b));
    logger_1.LOGGER.warn(`[MCP] Tool name exceeds ${maxLen} chars (incl. prefix budget) — shortening for strict clients: ${best}`);
...
function registerQueryTool(resAnno, server, authEnabled) {
    ...
    const queryHandler = async (rawArgs) => {
        ...
        return runQuery(resAnno, parsed.data, authEnabled, toolName);
    };
    server.registerTool(toolName, { title: toolName, description: desc, inputSchema }, queryHandler);
}
/**
 * Executes a list/query against an entity resource and shapes the MCP result.
 * Extracted from the query-tool handler so the consolidated read-bundle tool
 * can drive the identical code path (buildQuery + executeQuery + omission
 * filter) for any resource it dispatches to. `args` is the already-validated
 * query input (top/skip/select/orderby/where/odataFilter/q/expand/return/...).
 */
async function runQuery(resAnno, args, authEnabled, toolName) {
    const CDS = global.cds;
    const allKeys = Array.from(resAnno.properties.keys());
    logger_1.LOGGER.debug(`[EXECUTION TIME] Query tool: Looking for service: ${resAnno.serviceName}, available services:`, Object.keys(CDS.services || {}));
        ...
        try {
            ...
            const cls = classifyCapError(error, "QUERY_FAILED", toolName);
            return (0, utils_2.toolError)(cls.code, cls.msg, cls.extra, error);
        }
}
/**
...
function registerGetTool(resAnno, server, authEnabled) {
    ...
    const getHandler = async (args) => runGet(resAnno, args, authEnabled, toolName);
    server.registerTool(toolName, { title: toolName, description: desc, inputSchema }, getHandler);
}
/**
 * Reads a single entity row by key(s) and shapes the MCP result. Extracted
 * from the get-tool handler so the read-bundle tool shares the identical
 * key normalization, draft IsActiveEntity defaulting, $expand resolution and
 * omission filtering. `args` accepts single-key shorthand, a {key:value}
 * object, or a `value` field, plus optional `expand`.
 */
async function runGet(resAnno, args, authEnabled, toolName) {
        ...
        try {
            ...
            if (response == null) {
                return (0, utils_2.toolError)("NOT_FOUND", `No ${resAnno.target} found for keys ${JSON.stringify(keys)}`);
            }
            const result = (0, utils_2.applyOmissionFilter)(response, resAnno);
            return (0, utils_2.asMcpResult)(result);
        }
        catch (error) {
            const cls = classifyCapError(error, "GET_FAILED", toolName);
            return (0, utils_2.toolError)(cls.code, cls.msg, cls.extra, error);
        }
}
/**
...
function registerCreateTool(resAnno, server, authEnabled) {
    ...
    for (const [propName, cdsType] of resAnno.properties.entries()) {
        if (DRAFT_INTERNAL_FIELDS.has(propName))
            continue;
        ...
        inputSchema[propName] = (0, utils_2.determineMcpParameterType)(cdsType, propName, `${resAnno.serviceName}.${resAnno.target}`)
            .optional()
            .describe(describeWritableField(resAnno, propName, cdsType));
    }
...
function registerDeleteTool(resAnno, server, authEnabled) {
    ...
    const inputSchema = {};
    const draftEnabled = isDraftEnabledRoot(resolveEntityDefinition(resAnno));
    // Keys required for deletion. On draft-enabled roots make IsActiveEntity
    // optional with default `true` so callers can delete active rows by
    // business key alone — the runtime handler already defaults the flag,
    // but the zod schema would reject the request before it gets there.
    for (const [k, cdsType] of resAnno.resourceKeys.entries()) {
        if (draftEnabled && k === "IsActiveEntity") {
            inputSchema[k] = zod_1.z
                .boolean()
                .optional()
                .default(true)
                .describe("Draft selector: true (default) deletes the active row, false discards the draft sibling.");
            continue;
        }
        inputSchema[k] = (0, utils_2.determineMcpParameterType)(cdsType).describe(`Key ${k}. ${resAnno.propertyHints.get(k) ?? ""}`);
    }
    ...
    const deleteHandler = async (args) => {
        ...
        // Active-row DELETE on a draft-enabled root is a valid operation —
        // CAP cascades the active row deletion through the normal pipeline,
        // and any pending draft for the same business key gets cleaned up by
        // the framework. DRAFT_REQUIRED here would leave callers with no
        // reachable path to permanently remove an active entity. Draft-state
        // DELETE (IsActiveEntity=false) is also honored and is equivalent to
        // draft-discard for that row.
 ...
 */
/**
 * Emits a single debug line on the `mcp` channel describing the draft
 * operation about to run. Opt-in noise — enable with `DEBUG=mcp`.
...
function logDraftOp(op, resAnno, keysOrData) {
    try {
        ...
        logger_1.LOGGER.debug(`[draft-${op}] entity=${resAnno.serviceName}.${resAnno.target} keys=${JSON.stringify(keysOrData ?? {})} user=${user}`);
        // Full principal snapshot on every draft op for on-call diagnosis.
...
function logDraftTxDispatch(op, outerUserId, tx) {
    try {
        ...
        logger_1.LOGGER.debug(`[draft-${op}] dispatch outer='${outerUserId ?? "<none>"}' tx='${txUserId}' inner='${innerUserId}' — this is the id CAP will stamp into DraftAdministrativeData.InProcessByUser`);
    }
    catch { }
}
/**
 * Writes one line on the `mcp` channel describing the principal the
 * plugin will pass to `svc.tx({user})`. Opt-in via `DEBUG=mcp`.
 *
...
function logAuthContext(label) {
    try {
        ...
        if (!ctx) {
            logger_1.LOGGER.debug(`[${label}] cds.context is undefined — request did NOT go through CAP's context middleware. The tool handler is running outside the authenticated request scope.`);
            return;
        }
        const u = ctx.user;
        if (!u) {
            logger_1.LOGGER.debug(`[${label}] cds.context.user is undefined — auth middleware did not populate a principal. CAP will fall back to its default (usually 'anonymous').`);
            return;
        }
        const roles = u.roles ? Object.keys(u.roles) : [];
        logger_1.LOGGER.debug(`[${label}] caller id='${u.id}' privileged=${!!u._is_privileged} anonymous=${!!u._is_anonymous} tenant='${u.tenant ?? ctx.tenant ?? "<none>"}' roles=[${roles.join(", ")}]`);
    }
    catch { }
}
...
/**
 * Resolves the caller's own draft row for a given business key, disambiguating
 * the case where a stale foreign draft shares the same business key. Fiori's
 * OData read path scopes draft lookups by `DraftAdministrativeData.InProcessByUser`
 * (lean-draft.js:1468), so the UI picks the correct row even when an orphaned
 * DraftAdministrativeData entry from another user exists. CAP's PATCH / SAVE /
 * CANCEL paths do NOT apply the same scope — `SELECT.one.from(draftsRef)` at
 * lean-draft.js:1091 (and analogous code in save/cancel) takes the first
 * matching row. Without this pre-resolution, `draft-patch` against an orphan's
 * business key returns `DRAFT_LOCKED` even when the caller legitimately owns
 * their own draft — which is the exact shape of the EAM production trace.
 *
 * Returns:
 *   - ownDraftUUID: caller's DraftUUID if a matching row exists; else null.
 *   - foreignHolder: InProcessByUser of any foreign draft at the same key,
 *     for richer error messages. null if no foreign row.
 *
 * Privileged callers skip user-scoping — CAP bypasses the lock check for them
 * anyway, so ambiguity there is harmless.
 */
/**
 * Converts a `{k1: v1, k2: v2}` key object into a CQN where-array so it can
 * ride inside an entity ref (`{ref: [{id, where: [...]}]}`). Path-segment
 * WHERE travels with the entity through CAP's query rewriting (notably
 * `_redirectRefToDrafts` in lean-draft), whereas a top-level `.where(...)`
 * does not. Use this whenever the draft runtime needs to honour the filter.
 */
function keysToWhere(keys) {
    const where = [];
    for (const [k, v] of Object.entries(keys)) {
        if (where.length > 0)
            where.push("and");
        where.push({ ref: [k] }, "=", { val: v });
    }
    return where;
}
async function resolveCallerDraft(svc, resAnno, businessKeys, authEnabled) {
    const CDS = global.cds;
    const { SELECT } = CDS.ql;
    const currentUserId = CDS.context?.user?.id;
    const isPrivileged = !!CDS.context?.user?._is_privileged;
    // Query at the DB level (`cds.tx`) rather than through the service
    // (`svc.tx`) — the service's own draft read handlers filter out rows even
    // for privileged users in some code paths, which would hide the foreign
    // orphans we specifically need to detect. `cds.tx` hits the underlying DB
    // tables directly; no user scoping, no handler chain.
    const qualifiedTarget = resAnno.target.includes(".")
        ? resAnno.target
        : `${resAnno.serviceName}.${resAnno.target}`;
    const rows = await CDS.tx({ user: CDS.User.privileged }, (tx) => tx.run(SELECT.from(`${qualifiedTarget}.drafts`)
        .columns("DraftAdministrativeData_DraftUUID")
        .where(businessKeys)));
    if (!rows || rows.length === 0) {
        return { ownDraftUUID: null, foreignHolder: null };
    }
    const uuids = rows
        .map((r) => r.DraftAdministrativeData_DraftUUID)
        .filter(Boolean);
    if (uuids.length === 0) {
        return { ownDraftUUID: null, foreignHolder: null };
    }
    // DraftAdministrativeData is a DB-level table not projected through the
    // app service; query it via cds.tx to avoid "service does not expose this
    // entity" errors.
    const admin = await CDS.tx({ user: CDS.User.privileged }, (tx) => tx.run(SELECT.from("DRAFT.DraftAdministrativeData")
        .columns("DraftUUID", "InProcessByUser")
        .where({ DraftUUID: { in: uuids } })));
    const holderByUUID = new Map((admin || []).map((a) => [a.DraftUUID, a.InProcessByUser]));
    // When auth is disabled (or the caller is already running privileged),
    // there is no meaningful "current user" to scope against — every draft
    // at this key is effectively ours. Pick the first row.
    if (isPrivileged || !authEnabled) {
        return { ownDraftUUID: uuids[0], foreignHolder: null };
    }
    let ownDraftUUID = null;
    let foreignHolder = null;
    for (const u of uuids) {
        const holder = holderByUUID.get(u);
        if (holder === currentUserId) {
            ownDraftUUID = u;
        }
        else if (holder && !foreignHolder) {
            foreignHolder = holder;
        }
    }
    return { ownDraftUUID, foreignHolder };
}
/**
 * Pre-dispatch DRAFT_LOCKED response — used when `resolveCallerDraft` shows
 * that only a foreign draft exists at the requested business key. The message
 * mirrors the one `classifyCapError` emits for `DRAFT_LOCKED_BY_ANOTHER_USER`
 * so on-call sees a consistent shape whether the lock was caught before or
 * during the CAP dispatch.
 */
function draftLockedError(toolName, holder) {
    const CDS = global.cds;
    const caller = CDS.context?.user?.id ?? "<unknown — cds.context.user not populated>";
    const callerIsPrivileged = !!CDS.context?.user?._is_privileged;
    ...
    const callerHint = callerIsPrivileged
        ...
        : ` (you are '${caller}')`;
    const diagnostic = holder && caller && holder !== caller
        ? " Root cause: a foreign draft from another user shares this business key. Your own draft either does not exist yet or was already discarded. The Fiori UI scopes draft lookups to the current user and would pick your row; this plugin previously inherited CAP's unscoped lookup. Call draft-new to open your own, or ask the holder (or an admin) to discard the orphan."
        : "";
    const discardName = toolName.replace(/_[^_]+$/, "_draft-discard");
    const msg = `${toolName}: draft is locked by another user${holderHint}${callerHint}.${diagnostic} Retry as that user or wait for cds.env.drafts.cancellationTimeout (default 15 min), then ${discardName} to release.`;
    return (0, utils_2.toolError)("DRAFT_LOCKED", msg);
}
// Appended to every draft-* tool description so the LLM always has the
// lifecycle semantics in context when deciding which draft tool to call.
const DRAFT_LIFECYCLE_HINT = " Draft lifecycle: a draft is a private per-user working copy — other users cannot see it. draft-new creates a brand-new entity that has no active version yet; draft-activate saves it so all users see the active row. For an existing active row, any authorized user can call draft-edit to open their own editable copy — while that draft exists the active row is locked, so no other user can start a concurrent draft-edit on it. Only one user can hold a draft of a given active row at a time; the holder must draft-activate (save) or draft-discard (abandon) to release the lock.";

/**
 * B-NEW-1 helper. For draft-enabled entities with an Integer primary key and
 * no `@cds.on.insert` stamping, SQLite's `.drafts` table rowid counter is
 * not seeded from the active table — so draft-new hands out ID=1 on every
 * boot, colliding with the existing active row on activate. Pre-compute
 * MAX(active, drafts) + 1 and stamp it into `data` so the INSERT lands on
 * a free slot. Silent no-op when:
 *   - the entity has no integer key (UUID keys are safe)
 *   - the key is already CAP-managed (`@cds.on.insert` present)
 *   - the caller supplied an explicit value
 *   - the underlying query fails (best-effort; don't block draft-new)
 */
async function preAssignIntegerKey(resAnno, entityDef, data) {
    if (!entityDef?.elements) return;
    const keyEntries = Object.entries(entityDef.elements)
        .filter(([_, el]) => el?.key === true);
    if (keyEntries.length !== 1) return;
    const [keyName, keyEl] = keyEntries[0];
    if (data[keyName] !== undefined) return;
    const type = String(keyEl.type || "");
    const INTEGER_TYPES = new Set(["cds.Integer", "cds.Integer64", "cds.Int16", "cds.Int32", "cds.Int64", "cds.UInt8"]);
    if (!INTEGER_TYPES.has(type)) return;
    const elementKeys = new Map(Object.keys(keyEl).map((k) => [k.toLowerCase(), k]));
    if (elementKeys.has("@cds.on.insert")) return;
    const CDS = global.cds;
    const { SELECT } = CDS.ql || {};
    if (!SELECT) return;
    const activeTarget = `${resAnno.serviceName}.${resAnno.target}`;
    const draftsTarget = `${activeTarget}.drafts`;
    try {
        const nextId = await CDS.tx({ user: CDS.User.privileged }, async (dbTx) => {
            const activeRows = await dbTx.run(SELECT.from(activeTarget).columns(`max(${keyName}) as m`));
            const draftRows = await dbTx.run(SELECT.from(draftsTarget).columns(`max(${keyName}) as m`));
            const activeMax = Number(Array.isArray(activeRows) ? activeRows[0]?.m : activeRows?.m) || 0;
            const draftMax = Number(Array.isArray(draftRows) ? draftRows[0]?.m : draftRows?.m) || 0;
        ...
        // Intentional: seed query is best-effort. Schemas we can't read here
        // (exotic projections, HANA sequences) will fall through to CAP's
        // native ID assignment, which on HANA already does the right thing.
    }
}
/**
 * Writes nested composition children against a parent's `.drafts` table,
 * propagating the parent's `DraftAdministrativeData_DraftUUID` and the
 * back-reference FK columns onto newly-inserted child rows.
 *
 * The draft-patch path runs at the DB layer to keep the `DraftUUID` filter
 * in the WHERE clause (lean-draft's `_cleanseWhere` would strip it at the
 * service layer). The DB-layer UPDATE cannot, however, deep-write nested
 * compositions with the right draft bookkeeping — so any new item reached
 * through the `items: [...]` array hits `SQLITE_CONSTRAINT_NOTNULL` on
 * `DraftAdministrativeData_DraftUUID`. We reconstruct what lean-draft's
 * NEW handler does for deep insert, but scoped to an existing parent draft.
 *
 * Each item with a child PK already present in the drafts table is treated
 * as an update (scalar fields applied); items without a matching draft row
 * are INSERTed with UUID + parent-FK propagation.
 */
async function applyDraftCompositionPatch(dbTx, parentResAnno, parentDef, compName, compElement, items, parentKeys, parentDraftUUID) {
    const CDS = global.cds;
    const { INSERT, UPDATE, SELECT } = CDS.ql || {};
    if (!INSERT || !UPDATE || !SELECT) return;
    const childTargetFqn = compElement?.target;
    if (!childTargetFqn) return;
    const childDef = CDS.model?.definitions?.[childTargetFqn];
    if (!childDef?.elements) return;
    // Locate the child's back-reference Association — its name determines
    // the flat FK column names on the child (`${backRef}_${parentKey}`).
    // Prefer the on-clause hint (`items.parent = $self`); fall back to
    // scanning the child for an Association whose target matches the parent.
    const parentFqn = `${parentResAnno.serviceName}.${parentResAnno.target}`;
    let backRefName = null;
    if (Array.isArray(compElement.on)) {
        for (const part of compElement.on) {
            if (part && Array.isArray(part.ref) && part.ref[0] === compName && part.ref[1]) {
                backRefName = part.ref[1];
                break;
            }
        }
    }
    if (!backRefName) {
        const found = Object.entries(childDef.elements).find(([, v]) => v?.type === "cds.Association" &&
            (v?.target === parentFqn || v?.target === parentDef?.name));
        backRefName = found?.[0] ?? null;
    }
    // Resolve the child's drafts table. Prefer the service-scoped name so
    // CAP's draft runtime sees the expected handlers; fall back to the raw
    // CSN target if the service doesn't re-project the child.
    const childShortName = childTargetFqn.split(".").pop();
    const serviceScopedChild = `${parentResAnno.serviceName}.${childShortName}`;
    const childDraftsTarget = CDS.model?.definitions?.[serviceScopedChild]
        ? `${serviceScopedChild}.drafts`
        : `${childTargetFqn}.drafts`;
    const childKeys = Object.entries(childDef.elements)
        .filter(([, v]) => v?.key === true)
        .map(([k]) => k);
    const backRefFks = {};
    if (backRefName) {
        for (const [pkName, pkVal] of Object.entries(parentKeys)) {
            const col = `${backRefName}_${pkName}`;
            if (childDef.elements[col]) {
                backRefFks[col] = pkVal;
            }
        }
    }
    for (const rawItem of items) {
        ...
        for (const [k, v] of Object.entries(rawItem)) {
            if (DRAFT_INTERNAL_FIELDS.has(k)) continue;
            if (!childDef.elements[k]) continue;
            fields[k] = coerceScalarForField(v, k, childDef);
        }
        let rowExists = false;
        if (childKeys.length > 0 && childKeys.every((k) => fields[k] !== undefined)) {
            const whereKeys = {};
            for (const k of childKeys) whereKeys[k] = fields[k];
            const existing = await dbTx.run(SELECT.from(childDraftsTarget).where(whereKeys));
            const hit = Array.isArray(existing) ? existing.length > 0 : Boolean(existing);
            if (hit) {
                rowExists = true;
                const setFields = { ...fields };
                for (const k of childKeys) delete setFields[k];
                if (Object.keys(setFields).length > 0) {
                    await dbTx.run(UPDATE(childDraftsTarget).set(setFields).where(whereKeys));
                }
            }
        }
        if (!rowExists) {
            const payload = {
                ...backRefFks,
                ...fields,
                DraftAdministrativeData_DraftUUID: parentDraftUUID,
                IsActiveEntity: false,
                HasActiveEntity: false,
                HasDraftEntity: false,
            };
            await dbTx.run(INSERT.into(childDraftsTarget).entries(payload));
        }
    }
}
/**
...
function registerDraftNewTool(resAnno, server, authEnabled) {
    ...
    const hint = constructHintMessage(resAnno, ["draft-new", "create"]);
    const desc = `Resource description: ${resAnno.description}. Create a new draft of ${resAnno.target} (IsActiveEntity=false). Follow up with ${nameFor(resAnno, "draft-patch")} to edit fields and ${nameFor(resAnno, "draft-activate")} to publish.${hint}${DRAFT_LIFECYCLE_HINT}`;
    const handler = async (args) => {
        ...
        // B-NEW-1 mitigation: draft-enabled entities whose primary key is an
        // Integer without `@cds.on.insert` rely on the DB's rowid counter.
        // On SQLite the `.drafts` shadow table is not seeded from the active
        // table, so its counter resets to 1 on every boot — activate then
        // fails with UNIQUE constraint against the active row that already
        // has ID=1. Pre-compute MAX(active, drafts) + 1 and stamp it into
        // `data` so CAP inserts a collision-free value. We only do this
        // when the caller didn't supply the key (CAP best practice for
        // draft-enabled entities is UUIDs; this is a pragmatic safety net).
        await preAssignIntegerKey(resAnno, def, data);
        ...
        try {
            ...
            const created = await withTimeout(svc.tx({ user: (0, utils_1.getAccessRights)(authEnabled) }, (tx) => {
                logDraftTxDispatch("new", outerUserIdNew, tx);
                return tx.send("NEW", `${resAnno.target}.drafts`, data);
            }), TIMEOUT_MS, toolName);
            // Re-read the just-created draft row by DraftUUID so auto-generated
            // keys (`@cds.on.insert = $uuid`, `@core.Computed`, DB defaults)
            // round-trip to the caller — CAP's NEW response is sparse and
            // frequently omits generated keys, which breaks any follow-up
            // draft-patch / draft-activate call addressing the new row. Runs
            // as a *separate* tx after the NEW commit so we don't deadlock
            // SQLite's single writer against the still-open outer tx.
            const uuid = created?.DraftAdministrativeData_DraftUUID
                ?? (created && typeof created === "object" ? created.DraftAdministrativeData?.DraftUUID : undefined);
            let merged = { ...data, ...(created && typeof created === "object" ? created : {}) };
            if (uuid) {
                try {
                    const { SELECT } = global.cds.ql;
                    const draftsTargetNew = `${resAnno.serviceName}.${resAnno.target}.drafts`;
                    const rows = await global.cds.tx({ user: global.cds.User.privileged }, (dbTx) => dbTx.run(SELECT.from(draftsTargetNew).where({ DraftAdministrativeData_DraftUUID: uuid })));
                    const row = Array.isArray(rows) ? rows[0] : rows;
                    if (row && typeof row === "object")
                        merged = { ...merged, ...row };
                }
                catch {
                    // Re-read is best-effort. If the schema rejects the drafts-
                    // table WHERE (exotic key setups), fall back to the NEW
                    // response we already have instead of failing the call.
                }
            }
...
function registerDraftEditTool(resAnno, server, authEnabled) {
    ...
    const hint = constructHintMessage(resAnno, ["draft-edit", "update"]);
    const desc = `Resource description: ${resAnno.description}. Start editing ${resAnno.target} by key(s): ${keyList} — creates a draft copy of the active row. Follow up with ${nameFor(resAnno, "draft-patch")} and ${nameFor(resAnno, "draft-activate")}.${hint}${DRAFT_LIFECYCLE_HINT}`;
    const handler = async (args) => {
        const svc = await resolveServiceInstance(resAnno.serviceName);
        if (!svc) {
            return (0, utils_2.toolError)("ERR_MISSING_SERVICE", `Service not found: ${resAnno.serviceName}`);
...
function registerDraftPatchTool(resAnno, server, authEnabled) {
    ...
    const keyList = Array.from(resAnno.resourceKeys.keys()).filter((k) => k !== "IsActiveEntity").join(", ");
    const hint = constructHintMessage(resAnno, ["draft-patch", "update"]);
    const desc = `Resource description: ${resAnno.description}. Patch an existing draft of ${resAnno.target} by key(s): ${keyList}. Requires a draft to already exist — call ${nameFor(resAnno, "draft-new")} or ${nameFor(resAnno, "draft-edit")} first. Follow up with ${nameFor(resAnno, "draft-activate")} to publish.${hint}${DRAFT_LIFECYCLE_HINT}`;
    const handler = async (args) => {
        const CDS = global.cds;
        ...
        if (extracted.error)
            return extracted.error;
        // User-scope the draft lookup so a stale foreign draft sharing the
        // same business key can't hijack the PATCH target. See
        // `resolveCallerDraft` comments for the full lean-draft rationale.
        const resolved = await resolveCallerDraft(svc, resAnno, extracted.keys, authEnabled);
        if (!resolved.ownDraftUUID) {
            if (resolved.foreignHolder) {
                return draftLockedError(toolName, resolved.foreignHolder);
            }
            return (0, utils_2.toolError)("DRAFT_NOT_FOUND", `${toolName}: no draft found for key(s). Call ${nameFor(resAnno, "draft-new")} or ${nameFor(resAnno, "draft-edit")} first.`);
        }
        // `IsActiveEntity` is a virtual CSN element on the active-facing
        // entity; once CAP's draft runtime redirects our ref to the `.drafts`
        // sibling it becomes invalid in a path-segment WHERE expression. The
        // DraftUUID + business keys alone already pin us to a single row on
        // the drafts table.
        const keys = {
            ...extracted.keys,
            DraftAdministrativeData_DraftUUID: resolved.ownDraftUUID,
        };
        const updates = {};
        const def = resolveEntityDefinition(resAnno);
        // Extract composition children from args up front. They can't go
        // through the scalar DB-layer UPDATE: CAP's native handler would
        // deep-write them but lives at the service layer; the DB-layer
        // UPDATE won't propagate `DraftAdministrativeData_DraftUUID` onto
        // newly-inserted child rows, and SQLite rejects them with
        // NOT NULL. We handle them below via `applyDraftCompositionPatch`.
        const compositionPayloads = new Map();
        if (def?.elements) {
            for (const [elName, el] of Object.entries(def.elements)) {
                if (el?.type !== "cds.Composition") continue;
                if (!Array.isArray(args[elName])) continue;
                compositionPayloads.set(elName, { element: el, items: args[elName] });
            }
        }
        for (const [propName, cdsType] of resAnno.properties.entries()) {
            if (resAnno.resourceKeys.has(propName))
                continue;
            if (compositionPayloads.has(propName))
                continue;
            const isAssociation = String(cdsType).toLowerCase().includes("association");
            if (isAssociation) {
                for (const fk of expandAssociationKeys(def, propName)) {
        ...
        if (Object.keys(updates).length === 0 && compositionPayloads.size === 0) {
        ...
        // Bypass CAP's lean-draft PATCH handler entirely: its `_cleanseWhere`
        // strips both `IsActiveEntity` and `DraftAdministrativeData_DraftUUID`
        // from our query (both are in DRAFT_ELEMENTS — lean-draft.js:71), so
        // the DraftUUID filter that pins us to the caller's own row would be
        // lost. Instead we UPDATE the `.drafts` sibling directly at the DB
        // layer — `resolveCallerDraft` has already validated ownership, and
        // the sibling table's WHERE accepts the DraftUUID column literally.
        // Re-read goes through the service layer so the returned shape
        // matches what an active-facing read would produce.
        const draftsTarget = `${resAnno.serviceName}.${resAnno.target}.drafts`;
        try {
            const row = await withTimeout((async () => {
                // UPDATE runs at the DB layer (no service-level read filter to
                // dodge here — we already resolved the target row above), so we
                // use the real caller's context. That keeps `@cds.on.update =
                // $user` stamping honest instead of smearing "privileged" onto
                // every draft patch. Scalar UPDATE + composition deep-writes
                // share one DB tx so they roll back together on any failure.
                await global.cds.tx({ user: (0, utils_1.getAccessRights)(authEnabled) }, async (dbTx) => {
                    if (Object.keys(updates).length > 0) {
                        await dbTx.run(UPDATE(draftsTarget).set(updates).where(keys));
                    }
                    for (const [compName, { element, items }] of compositionPayloads) {
                        await applyDraftCompositionPatch(dbTx, resAnno, def, compName, element, items, extracted.keys, resolved.ownDraftUUID);
                    }
                });
                return svc.tx({ user: (0, utils_1.getAccessRights)(authEnabled) }, (tx) => {
                    logDraftTxDispatch("patch", outerUserIdPatch, tx);
                    return tx.run(SELECT.from(resAnno.target).where({
                        ...extracted.keys,
                        IsActiveEntity: false,
                    }));
                });
...
function registerDraftActivateTool(resAnno, server, authEnabled) {
    ...
    const keyList = Array.from(resAnno.resourceKeys.keys()).filter((k) => k !== "IsActiveEntity").join(", ");
    const hint = constructHintMessage(resAnno, ["draft-activate", "update"]);
    const desc = `Resource description: ${resAnno.description}. Activate (publish) the pending draft of ${resAnno.target} by key(s): ${keyList}. Requires a draft to exist; otherwise returns DRAFT_NOT_FOUND.${hint}${DRAFT_LIFECYCLE_HINT}`;
    const handler = async (args) => {
        const svc = await resolveServiceInstance(resAnno.serviceName);
        if (!svc) {
            return (0, utils_2.toolError)("ERR_MISSING_SERVICE", `Service not found: ${resAnno.serviceName}`);
        ...
        if (extracted.error)
            return extracted.error;
        // Scope the SAVE target to the caller's own DraftUUID — otherwise an
        // orphan draft sharing the same business key can divert the activate.
        const resolved = await resolveCallerDraft(svc, resAnno, extracted.keys, authEnabled);
        if (!resolved.ownDraftUUID) {
            if (resolved.foreignHolder) {
                return draftLockedError(toolName, resolved.foreignHolder);
            }
            return (0, utils_2.toolError)("DRAFT_NOT_FOUND", `${toolName}: no draft found to activate. Call ${nameFor(resAnno, "draft-new")} first.`);
        }
        const saveKeys = {
            ...extracted.keys,
            DraftAdministrativeData_DraftUUID: resolved.ownDraftUUID,
        };
        logDraftOp("activate", resAnno, saveKeys);
        try {
            const response = await withTimeout(svc.tx({ user: (0, utils_1.getAccessRights)(authEnabled) }, (tx) => tx.send("SAVE", resAnno.target, saveKeys)), TIMEOUT_MS, toolName);
            return (0, utils_2.asMcpResult)((0, utils_2.applyOmissionFilter)(response, resAnno) ?? {});
        }
        catch (error) {
...
function registerDraftDiscardTool(resAnno, server, authEnabled) {
    ...
    const keyList = Array.from(resAnno.resourceKeys.keys()).filter((k) => k !== "IsActiveEntity").join(", ");
    // Deliberately does NOT fall back to the "delete" hint — the two operations
    // target different things. Delete removes the active row; discard only
    // removes the pending draft. Mixing hints leads to misleading guidance
    // like "Irreversible; prefer setting stock to 0 first" on discard.
    const hint = constructHintMessage(resAnno, ["draft-discard"]);
    const desc = `Resource description: ${resAnno.description}. Discard the pending draft of ${resAnno.target} by key(s): ${keyList}. The active row is untouched. This operation cannot be undone.${hint}${DRAFT_LIFECYCLE_HINT}`;
    const handler = async (args) => {
        const svc = await resolveServiceInstance(resAnno.serviceName);
        if (!svc) {
            return (0, utils_2.toolError)("ERR_MISSING_SERVICE", `Service not found: ${resAnno.serviceName}`);
        ...
        if (extracted.error)
            return extracted.error;
        // Scope the CANCEL target to the caller's own draft — otherwise a
        // discard call could silently wipe out a stranger's draft that
        // happens to share the business key.
        const resolved = await resolveCallerDraft(svc, resAnno, extracted.keys, authEnabled);
        if (!resolved.ownDraftUUID) {
            if (resolved.foreignHolder) {
                return draftLockedError(toolName, resolved.foreignHolder);
            }
            return (0, utils_2.toolError)("DRAFT_NOT_FOUND", `${toolName}: no draft to discard for key(s).`);
        }
        // CANCEL targets `.drafts` directly, so IsActiveEntity is virtual
        // and must not appear in the key dict — CAP rejects virtual elements
        // in query expressions.
        const cancelKeys = {
            ...extracted.keys,
            DraftAdministrativeData_DraftUUID: resolved.ownDraftUUID,
        };
        logDraftOp("discard", resAnno, cancelKeys);
        try {
            ...
            await withTimeout(svc.tx({ user: (0, utils_1.getAccessRights)(authEnabled) }, (tx) => tx.send("CANCEL", `${resAnno.target}.drafts`, cancelKeys)), TIMEOUT_MS, toolName);
...
function registerDraftUpsertTool(resAnno, server, authEnabled) {
    ...
    const handler = async (args) => {
        ...
        // B-NEW-1 mitigation — same reasoning as registerDraftNewTool.
        await preAssignIntegerKey(resAnno, def, data);
...
function buildQuery(CDS, args, resAnno, propKeys) {
    ...
    // OData $filter string used by the consolidated read-bundle tool, which
    // can't build per-entity zod field enums for the structured `where` path.
    // Parsed to a CQN where-clause via CAP's own OData parser — LLMs are far
    // more fluent in OData filter syntax ("stock gt 0 and contains(name,'x')")
    // than CQL, and CDS.odata.parse yields the same {ref}/{val}/{func} nodes
    // the structured path produces. The per-entity query tool keeps using the
    // structured `where` array below.
    if (typeof args.odataFilter === "string" && args.odataFilter.trim().length > 0) {
        let parsedWhere;
        try {
            // The entity segment is syntactic only (the parser does not resolve
            // it against the model); the field refs come from the $filter.
            const parsed = CDS.odata.parse(`Entity?$filter=${args.odataFilter}`);
            parsedWhere = parsed?.SELECT?.where;
        }
        catch (e) {
            throw new Error(`$filter parse failed: ${e?.message || String(e)}`);
        }
        if (Array.isArray(parsedWhere) && parsedWhere.length) {
            qy = qy.where(parsedWhere);
        }
    }
...
function constructHintMessage(resAnno, wrapAction) {
    ...
    // `wrapAction` may be either a single key or an ordered candidate list.
    // For draft-* tools we pass `["draft-discard", "delete"]` or similar so
    // authors can customise per draft-mode but fall back to the CRUD hint
    // only when it's semantically compatible. draft-discard on its own must
    // NOT fall back to "delete" — the delete hint ("irreversible", "prefer
    // setting stock to 0") is plainly wrong for discarding a pending draft.
    const candidates = Array.isArray(wrapAction) ? wrapAction : [wrapAction];
    for (const key of candidates) {
        const val = resAnno.wrap.hint[key];
...
// ---------------------------------------------------------------------------
// Read bundling (opt-in): one consolidated "read" tool per group instead of a
// query + get tool per entity. See lib/config/loader.js `read_bundle`.
// ---------------------------------------------------------------------------
/**
 * Resolves the bundle group key for a resource. "service" groups every entity
 * under its CAP service (default); "annotation" groups by @mcp.wrap.group,
 * falling back to the service name when an entity carries no group.
 * @param {import("../annotations/structures").McpResourceAnnotation} resAnno
 * @param {object} config - loaded plugin configuration
 */
function groupKeyFor(resAnno, config) {
    if (config?.read_bundle?.group_by === "annotation") {
        const g = resAnno.wrap?.group;
        if (typeof g === "string" && g.length > 0)
            return g;
    }
    return resAnno.serviceName;
}
/**
 * The stable identifier a bundle exposes for a member resource (the value the
 * caller passes as `resource`). Prefers @mcp.name, falling back to the legacy
 * Service_Entity shape — identical to the per-entity tool-name stem.
 */
function resourceIdFor(resAnno) {
    if (typeof resAnno.name === "string" && resAnno.name.length > 0)
        return resAnno.name;
    const entityName = String(resAnno.target || "Entity").split(".").pop();
    const svcName = String(resAnno.serviceName || "Service").split(".").pop();
    return `${svcName}_${entityName}`;
}
/**
 * Builds the consolidated read tool name for a group, e.g. "MailService_read".
 * Mirrors toolNameFor's 64-char prefix-budget guard so the final (possibly
 * prefixed) name stays within the MCP tool-name limit.
 */
function bundleToolName(groupKey, suffix) {
    const shortKey = String(groupKey || "Group").split(".").pop();
    const name = `${shortKey}_${suffix || "read"}`;
    const prefixOverhead = (0, loader_1.applyToolNamePrefix)("").length;
    const maxLen = 64 - prefixOverhead;
    if (name.length <= maxLen)
        return name;
    logger_1.LOGGER.warn(`[MCP] Read-bundle tool name exceeds ${maxLen} chars (incl. prefix budget) — shortening for strict clients: ${name}`);
    return name.slice(0, maxLen);
}
/** Scalar (non-association, non-omitted) field names of a resource. */
function scalarFieldsOf(resAnno) {
    return Array.from(resAnno.properties.entries())
        .filter(([k, cdsType]) => !String(cdsType).toLowerCase().includes("association") &&
        !resAnno.omittedFields?.has(k))
        .map(([name]) => name);
}
/**
 * Composes the bundle tool description: an enumerated list of the bundled
 * resources so the LLM can pick the right `resource`. Per-resource @mcp.wrap
 * hints are preserved via constructHintMessage.
 * @param {string} groupKey
 * @param {{resAnno: import("../annotations/structures").McpResourceAnnotation, readModes: string[]}[]} members
 */
function buildReadBundleDescription(groupKey, members) {
    const shortKey = String(groupKey || "").split(".").pop();
    const lines = members.map((m) => {
        const id = resourceIdFor(m.resAnno);
        const keys = Array.from(m.resAnno.resourceKeys?.keys() ?? []).join(", ") || "(none)";
        const hint = constructHintMessage(m.resAnno, "query");
        const description = m.resAnno.description ? `${m.resAnno.description} ` : "";
        return `- ${id} — ${description}keys: ${keys}. [${m.readModes.join(", ")}]${hint}`;
    });
    return (`Consolidated READ access for the ${shortKey} resources. Set \`resource\` to one of the ` +
        `resources below and \`mode\` to "query" (list/filter many rows) or "get" (single row by key). ` +
        `For a resource's exact fields, keys and example payloads, call cap_describe_model with that resource name.\n\n` +
        `Resources:\n${lines.join("\n")}`);
}
/**
 * Registers ONE consolidated read tool for a group of resources. Replaces the
 * per-entity query/get tools (those are skipped at the factory when bundling is
 * enabled). Dispatches to the shared runQuery/runGet so behaviour is identical
 * to the per-entity tools; field-level detail comes from cap_describe_model.
 * @param {string} groupKey
 * @param {{resAnno: import("../annotations/structures").McpResourceAnnotation, readModes: string[]}[]} members
 * @param {any} server
 ...
 * @param {object} config
 */
function registerReadBundleTool(groupKey, members, server, authEnabled, config) {
    if (!members || members.length === 0)
        return;
    const suffix = config?.read_bundle?.tool_suffix || "read";
    const toolName = bundleToolName(groupKey, suffix);
    const membersByResource = new Map();
    for (const m of members) {
        membersByResource.set(resourceIdFor(m.resAnno), m);
    }
    const resourceIds = Array.from(membersByResource.keys());
    const inputZod = zod_1.z
        .object({
        resource: zod_1.z
            .enum(resourceIds)
            .describe(`Which resource to read. One of: ${resourceIds.join(", ")}. Call cap_describe_model with the resource name for its fields & keys.`),
        mode: zod_1.z
            .enum(["query", "get"])
            .default("query")
            .describe("query = list/filter many rows; get = single row by key"),
        key: zod_1.z
            .union([zod_1.z.string(), zod_1.z.number(), zod_1.z.record(zod_1.z.any())])
            .optional()
            .describe("get mode only: single-key value, or {keyName: value} for composite keys."),
        filter: zod_1.z
            .string()
            .max(2000)
            .optional()
            .describe('query mode: an OData $filter expression, e.g. "stock gt 0 and price le 50", "contains(title,\'sql\')", "genre_ID in (1,2)". Operators: eq ne gt ge lt le, and/or/not, parentheses, and functions like contains/startswith/endswith/tolower. Use foreign-key fields (author_ID), not association names. For free-text search use `q`.'),
        select: zod_1.z.array(zod_1.z.string()).optional().describe("Scalar field names to return."),
        orderby: zod_1.z
            .array(zod_1.z.string())
            .optional()
            .describe('Sort fragments, e.g. ["price desc", "title"].'),
        top: zod_1.z.number().int().min(1).max(MAX_TOP).default(25).describe("Rows (default 25)"),
        skip: zod_1.z.number().int().min(0).default(0).describe("Offset"),
        expand: zod_1.z
            .string()
            .max(2000)
            .optional()
            .describe("OData $expand; call cap_describe_model for valid nav props on the chosen resource."),
        q: zod_1.z.string().optional().describe("Quick text search across string fields."),
        return: zod_1.z.enum(["rows", "count", "aggregate"]).default("rows").optional(),
        aggregate: zod_1.z
            .array(zod_1.z.object({
        ...
        IsActiveEntity: zod_1.z
            .boolean()
            .optional()
            .describe("Draft roots only: true = active rows, false = draft rows, omit = both."),
    })
        .strict();
    const inputSchema = {
        resource: inputZod.shape.resource,
        mode: inputZod.shape.mode,
        key: inputZod.shape.key,
        filter: inputZod.shape.filter,
        select: inputZod.shape.select,
        orderby: inputZod.shape.orderby,
        top: inputZod.shape.top,
        skip: inputZod.shape.skip,
        ...
        return: inputZod.shape.return,
        aggregate: inputZod.shape.aggregate,
        IsActiveEntity: inputZod.shape.IsActiveEntity,
    };
    const desc = buildReadBundleDescription(groupKey, members);
    const handler = async (rawArgs) => {
        const parsed = inputZod.safeParse(rawArgs);
        if (!parsed.success) {
            return (0, utils_2.toolError)("INVALID_INPUT", "Read arguments failed validation", { issues: parsed.error.issues });
        }
        const args = parsed.data;
        const member = membersByResource.get(args.resource);
        if (!member) {
            return (0, utils_2.toolError)("UNKNOWN_RESOURCE", `Unknown resource '${args.resource}'. One of: ${resourceIds.join(", ")}`);
        }
        const resAnno = member.resAnno;
        // Authoritative per-resource read gate. Entities in one bundle may carry
        // different @requires/@restrict — the list-time gate only checks "any
        // readable member", so enforce the specific resource here.
        if (authEnabled && resAnno.restrictions && resAnno.restrictions.length) {
            const user = global.cds?.context?.user;
            if (user) {
                const access = (0, utils_1.getWrapAccesses)(user, resAnno.restrictions);
                if (!access.canRead) {
                    return (0, utils_2.toolError)("FORBIDDEN", `Not authorized to read '${args.resource}'.`);
                }
            }
        }
        const mode = args.mode ?? "query";
        if (!member.readModes.includes(mode)) {
            return (0, utils_2.toolError)("MODE_NOT_ALLOWED", `Resource '${args.resource}' does not expose '${mode}'. Available: ${member.readModes.join(", ")}.`);
        }
        if (mode === "get") {
            const getArgs = args.key !== null &&
                typeof args.key === "object" &&
                !Array.isArray(args.key)
                ? { ...args.key, expand: args.expand }
                : { value: args.key, expand: args.expand };
            return runGet(resAnno, getArgs, authEnabled, toolName);
        }
        // query: validate referenced fields against the resolved resource.
        const scalar = new Set(scalarFieldsOf(resAnno));
        const badSelect = (args.select ?? []).filter((f) => !scalar.has(f));
        if (badSelect.length) {
            return (0, utils_2.toolError)("INVALID_FIELD", `Unknown select field(s) for '${args.resource}': ${badSelect.join(", ")}. Valid: ${[...scalar].join(", ")}`);
        }
        const orderby = [];
        for (const o of args.orderby ?? []) {
            const [field, dirRaw] = String(o).trim().split(/\s+/);
            const dir = String(dirRaw || "asc").toLowerCase() === "desc" ? "desc" : "asc";
            if (!scalar.has(field)) {
                return (0, utils_2.toolError)("INVALID_FIELD", `Unknown orderby field '${field}' for '${args.resource}'. Valid: ${[...scalar].join(", ")}`);
            }
            orderby.push({ field, dir });
        }
        const badAgg = (args.aggregate ?? []).filter((a) => !scalar.has(a.field));
        if (badAgg.length) {
            return (0, utils_2.toolError)("INVALID_FIELD", `Unknown aggregate field(s) for '${args.resource}': ${badAgg.map((a) => a.field).join(", ")}.`);
        }
        const isDraft = isDraftEnabledRoot(resolveEntityDefinition(resAnno));
        const queryArgs = {
            top: args.top,
            skip: args.skip,
            select: args.select,
            orderby: orderby.length ? orderby : undefined,
            odataFilter: args.filter,
            q: args.q,
            expand: args.expand,
            return: args.return,
            aggregate: args.aggregate,
            // IsActiveEntity is only a valid column on draft roots; forwarding it
            // for a plain entity would add a WHERE on a non-existent column.
            IsActiveEntity: isDraft ? args.IsActiveEntity : undefined,
        };
        return runQuery(resAnno, queryArgs, authEnabled, toolName);
    };
    server.registerTool(toolName, { title: toolName, description: desc, inputSchema }, handler);
}
