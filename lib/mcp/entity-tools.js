 ...
 * query input (top/skip/select/orderby/where/filterExpr/q/expand/return/...).
 */
...
function buildQuery(CDS, args, resAnno, propKeys) {
    ...
    // Raw OData-style $filter string used by the consolidated read-bundle tool,
    // which can't build per-entity zod field enums for the structured `where`
    // path. Parsed to CQN via CDS.parse.expr (parameterized, not string-
    // concatenated). The per-entity query tool keeps using `where` below.
    if (typeof args.filterExpr === "string" && args.filterExpr.trim().length > 0) {
        qy = qy.where(CDS.parse.expr(args.filterExpr));
    }
...
function registerReadBundleTool(groupKey, members, server, authEnabled, config) {
    ...
    const inputZod = zod_1.z
        ...
        filter: zod_1.z
            ...
            .max(2000)
            .optional()
            .describe('query mode: a boolean filter expression in CQL/CDS syntax, e.g. "stock > 0 and price <= 50". Use comparison operators (=, <>, >, >=, <, <=), and/or, and parentheses. Use foreign-key fields (author_ID), not association names. For free-text search use `q`.'),
