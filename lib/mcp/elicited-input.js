...
function constructElicitInput(params) {
    ...
    for (const [key, zodType] of Object.entries(params)) {
        ...
        // Surface the parameter's @mcp.hint (carried on the Zod schema as its
        // description via buildToolParameters) as the form field description so
        // the client-rendered popup is self-explanatory; fall back to the key.
        const hint = typeof zodType?.description === "string" && zodType.description.length > 0
            ? zodType.description
            : key;
