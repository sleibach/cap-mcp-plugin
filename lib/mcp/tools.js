...
function assignToolToServer(model, server, authEnabled) {
    ...
    const parameters = buildToolParameters(model.parameters, model.propertyHints, model.optionalParams);
    if (model.entityKey) {
...
function assignBoundOperation(params, model, server, authEnabled) {
    ...
    server.registerTool(model.name, {
        ...
        try {
            // Wrap in withTimeout so a hung user handler (e.g. an action
            // that awaits a stock pre-check that never resolves) can't
            // park the MCP server on that single request indefinitely.
            const response = await (0, entity_tools_1.withTimeout)(service.tx({ user: accessRights }).send({
                event: model.target,
                entity: model.entityKey,
                data: elicitationResult?.data ?? operationInput,
                params: [operationKeys],
            }), entity_tools_1.TIMEOUT_MS, model.name);
            return (0, utils_1.asMcpResult)(response);
        }
        catch (error) {
            const msg = `OPERATION_FAILED: ${error?.message ?? String(error)}`;
            return (0, utils_1.toolError)("OPERATION_FAILED", msg, undefined, error);
        }
    });
}
/**
...
function assignUnboundOperation(params, model, server, authEnabled) {
    ...
    server.registerTool(model.name, {
        ...
        const accessRights = (0, utils_2.getAccessRights)(authEnabled);
        try {
            const response = await (0, entity_tools_1.withTimeout)(service
                .tx({ user: accessRights })
                .send(model.target, elicitationResult?.data ?? args), entity_tools_1.TIMEOUT_MS, model.name);
            return (0, utils_1.asMcpResult)(response);
        }
        catch (error) {
            const msg = `OPERATION_FAILED: ${error?.message ?? String(error)}`;
 ...
 */
function buildToolParameters(params, propertyHints, optionalParams) {
    if (!params || params.size <= 0)
        return {};
    const result = {};
    for (const [k, v] of params.entries()) {
        let zodType = (0, utils_1.determineMcpParameterType)(v)?.describe(propertyHints.get(k) ?? "");
        // Mark non-mandatory operation parameters optional so agents aren't forced to supply them.
        if (zodType && optionalParams?.has(k)) {
