...
function createMcpServer(config, annotations) {
    ...
    // Apply the optional tool-name prefix (mcp.tool_name_prefix) to every registered
    // tool. Wrapping registerTool here is the single chokepoint that covers entity
    // wrappers, annotated actions/functions and the built-in cap_* tools, so two
    // servers exposing the same tools (e.g. dev + prod) can coexist in one client.
    const toolNamePrefix = (0, loader_1.getToolNamePrefix)();
    if (toolNamePrefix) {
        // Tool names are capped at 64 chars; the prefix eats into that budget (entity wrappers
        // already reserve it in toolNameFor). Keep prefixes short — "dev", "qa", "prod".
        const overhead = (0, loader_1.applyToolNamePrefix)("").length;
        if (overhead > 8) {
            logger_1.LOGGER.warn(`MCP tool_name_prefix "${toolNamePrefix}" adds ${overhead} chars to every tool name; keep it short (e.g. "dev", "qa", "prod") to stay within the 64-char tool-name limit.`);
        }
        const originalRegisterTool = server.registerTool.bind(server);
        server.registerTool = (name, ...rest) => {
            const prefixed = (0, loader_1.applyToolNamePrefix)(name);
            if (prefixed.length > 64) {
                logger_1.LOGGER.warn(`MCP tool name exceeds 64 chars after prefixing and may be rejected by strict clients: ${prefixed}`);
            }
            return originalRegisterTool(prefixed, ...rest);
        };
        logger_1.LOGGER.debug(`MCP tool names prefixed with "${toolNamePrefix}"`);
    }
    if (!annotations) {
    ...
    // Read bundling (opt-in): collect read-eligible resources per group during
    // the pass below, then register one consolidated read tool per group after
    // the loop. Write/draft tools still register per-entity inside the loop.
    const bundleEnabled = !!config.read_bundle?.enabled;
    const READ_MODES = new Set(["query", "get"]);
    ...
    for (const entry of annotations.values()) {
        if (entry instanceof structures_1.McpToolAnnotation) {
            ...
            if (enabled) {
                ...
                // When bundling is on, query/get are served by one consolidated
                // read tool per group (registered after this loop). Only the
                // write/draft modes register per-entity here. registerEntityWrappers
                // still auto-enables draft-* for draft roots even with an empty
                // mode list, so the per-entity write surface is unaffected.
                const readModes = bundleEnabled
                    ? modes.filter((m) => READ_MODES.has(m))
                    : [];
                const perEntityModes = bundleEnabled
                    ? modes.filter((m) => !READ_MODES.has(m))
                    : modes;
                if (bundleEnabled && readModes.length) {
                    const key = (0, entity_tools_1.groupKeyFor)(entry, config);
                    const bucket = bundleBuckets.get(key) ?? [];
                    bucket.push({ resAnno: entry, readModes });
                    bundleBuckets.set(key, bucket);
                }
                const before = snapshotToolNames(server);
                (0, entity_tools_1.registerEntityWrappers)(entry, server, authEnabled, perEntityModes, fullAccesses, bundleEnabled ? perEntityModes : undefined);
    ...
    // Register one consolidated read tool per bundle group (opt-in). Runs after
    // the per-entity pass so a group sees every contributing resource.
    for (const [groupKey, members] of bundleBuckets) {
        const before = snapshotToolNames(server);
        (0, entity_tools_1.registerReadBundleTool)(groupKey, members, server, authEnabled, config);
        installToolVisibilityGates(server, before, authEnabled, () => readBundleGate(members));
    }
    // Register MCP Apps (ui:// resources + _meta patches) for entities that
    // carry @UI.LineItem / @UI.FieldGroup Fiori annotations. Runs after all
    // tools are registered so the tool _meta patches can find them by name.
    (0, apps_1.registerApps)(server, annotations, config);
    return server;
}
/**
...
/**
 * List-time visibility predicate for a consolidated read-bundle tool. The
 * bundle lists when the caller can read at least one member; per-resource
 * enforcement happens at call time inside the handler. Returns null (no gate)
 * when no member is restricted, so unrestricted bundles always list. Members
 * with restrictions are required, so the bundle stays visible whenever an
 * unrestricted member exists or the caller can read any restricted one.
 */
function readBundleGate(members) {
    const anyRestricted = members.some((m) => m.resAnno.restrictions && m.resAnno.restrictions.length);
    if (!anyRestricted)
        return null;
    return (user) => members.some((m) => {
        if (!m.resAnno.restrictions || !m.resAnno.restrictions.length)
            return true;
        return !!(0, utils_1.getWrapAccesses)(user, m.resAnno.restrictions).canRead;
    });
}
