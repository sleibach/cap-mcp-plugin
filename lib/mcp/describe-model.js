        ...
        const snapshot = {
            ...
            user: u
                ? {
                    ...
                    // Roles granted at request time by the @sap/ams middleware (DCL policy
                    // evaluation). These are honoured by user.is() but never appear in u.roles,
                    // so without this field an AMS caller looks role-less in diagnostics.
                    ams_roles: u.amsRoles ?? null,
                    attr: u.attr ?? null,
                }
                : null,
...
function registerDescribeModelTool(server) {
    ...
    server.registerTool("cap_describe_model", {
        ...
        const describeEntity = (service, entity) => {
            ...
            // Use the same name synthesizer as real tool registration so
            // example names honour `@mcp.name` overrides and the 64-char
            // truncation — otherwise the LLM is shown a tool name that
            // doesn't exist when the entity carries a custom prefix.
            const entName = String(ent?.name || "entity");
            const svcPart = service || entName.split(".").slice(0, -1).join(".") || "Service";
            const mcpName = ent?.["@mcp.name"];
            const listName = (0, loader_1.applyToolNamePrefix)((0, entity_tools_1.toolNameFor)({ serviceName: svcPart, target: entName, mcpName }, "query"));
            const getName = (0, loader_1.applyToolNamePrefix)((0, entity_tools_1.toolNameFor)({ serviceName: svcPart, target: entName, mcpName }, "get"));
            return {
                service,
                entity: ent.name,
                keys,
