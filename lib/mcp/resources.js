    ...
    { title: model.target, description: detailedDescription }, async (uri, variables) => {
        ...
        try {
            ...
            for (const [k, v] of Object.entries(queryParameters)) {
                if (k === "expand") continue; // handled below
                ...
                switch (k) {
                    case "filter":
                        ...
                        continue;
                    case "select":
                        selectedColumns = validator.validateSelect(v);
                        continue;
                    case "orderby":
            ...
            // Resolve expand AFTER filter/select/orderby so the CQN columns
            // list can merge user-selected scalars with expand column specs.
            const runtimeCfg = (0, loader_1.loadConfiguration)();
            const modelDefs = (global.cds || cds).model?.definitions ?? {};
            const entityDef = modelDefs[`${model.serviceName}.${model.target}`] || modelDefs[model.target];
            const expandColumns = (0, expand_parser_1.resolveExpand)({
                userExpand: queryParameters.expand,
                defaultMode: model.expandMode ?? runtimeCfg.expand.default,
                entityDef,
                modelDefs,
                limits: { max_depth: runtimeCfg.expand.max_depth, max_breadth: runtimeCfg.expand.max_breadth },
            });
            if (selectedColumns && selectedColumns.length > 0) {
                query.columns([...selectedColumns, ...expandColumns]);
            }
            else if (expandColumns.length > 0) {
                query.columns(["*", ...expandColumns]);
            }
        }
        catch (error) {
    ...
    server.registerResource(model.name, `odata://${model.serviceName}/${model.name}`, { title: model.target, description: model.description }, async (uri, extra) => {
        ...
        try {
            const query = SELECT.from(model.target).limit(queryParameters.top
                ? validator.validateTop(queryParameters.top)
                : 100);
            // Apply implicit expand for static resources too, so naive callers
            // still get deep reads when the runtime/entity default says so.
            const runtimeCfg = (0, loader_1.loadConfiguration)();
