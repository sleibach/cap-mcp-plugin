...
class DbSessionStore {
    ...
    async createSession(config, annotations) {
        ...
        // Capture so rehydrate on later requests can reconstruct an identical server.
    ...
    async _rehydrate(sessionId) {
        if (!this._runtimeConfig) {
            throw new Error("DbSessionStore.rehydrate called before createSession captured runtime config");
        }
...
function createSessionStore(runtimeConfig) {
    ...
    let kind;
    if (explicitKind === "memory" || explicitKind === "db") {
        kind = explicitKind;
    }
    else {
        ...
        if (!hasDbBinding) {
            logger_1.LOGGER.debug(`[SESSION-STORE] No DB binding detected, defaulting to in-memory session store. Bind a database or set session_store.kind='db' for multi-instance deployments.`);
        }
    }
    if (kind === "db") {
        if (!hasDbBinding) {
            throw new Error(`[SESSION-STORE] session_store.kind='db' requires a DB binding (cds.env.requires.db). Bind a database or switch to session_store.kind='memory'.`);
        }
        const enableJson = resolveEnableJson();
        if (!enableJson) {
