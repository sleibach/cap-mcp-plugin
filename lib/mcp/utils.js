...
exports.toCdsTemporalString = toCdsTemporalString;
const constants_1 = require("./constants");
const zod_1 = require("zod");
/* @ts-ignore */
const cds = global.cds || require("@sap/cds"); // This is a work around for missing cds context
/**
 * Normalises a wire value (string, number, Date) to the CAP-expected string
 * representation for a given CDS temporal type. CAP's CSN validator rejects
 * JS Date instances on `cds.Date` fields (it expects "YYYY-MM-DD"), which is
 * why `z.coerce.date()` alone is not enough — we pair it with a transform
 * that produces the exact string CAP's wire-protocol layer emits.
 *
 * - `Date`      → "YYYY-MM-DD"             (strips time component)
 * - `DateTime`  → "YYYY-MM-DDTHH:MM:SSZ"   (seconds precision)
 * - `Timestamp` → full ISO 8601 with ms    (CAP default)
 */
function toCdsTemporalString(v, kind) {
    if (v === null || v === undefined) return v;
    // Treat the literal string "null" as the same intent as JSON null — agents
    // occasionally stringify the clear-this-field token, and Date("null")
    // parses to NaN which we'd otherwise reject. Better to normalise here
    // than silently store an arbitrary timestamp.
    ...
    if (Number.isNaN(d.getTime())) {
        throw new Error(`Invalid ${kind} value: ${JSON.stringify(v)}`);
    }
    switch (kind) {
        case "Date":
...
// Nullable branch on the union so the generated JSON Schema carries
// `{"type":"null"}` alongside string/number/date — agents need an explicit
// way to clear a nullable date field. Without it, they resort to the string
// "null" which previously produced spurious future timestamps (D-NEW-3).
const zDate = zod_1.z
    .union([zod_1.z.string(), zod_1.z.number(), zod_1.z.date(), zod_1.z.null()])
    .transform((v) => toCdsTemporalString(v, "Date"));
const zDateTime = zod_1.z
    .union([zod_1.z.string(), zod_1.z.number(), zod_1.z.date(), zod_1.z.null()])
    .transform((v) => toCdsTemporalString(v, "DateTime"));
const zTimestamp = zod_1.z
    .union([zod_1.z.string(), zod_1.z.number(), zod_1.z.date(), zod_1.z.null()])
    .transform((v) => toCdsTemporalString(v, "Timestamp"));
/** Accept numeric tool args from HTML forms / LLMs that stringify numbers. */
const zCoercedNumber = zod_1.z.preprocess((v) => {
...
function determineMcpParameterType(cdsType, key, target) {
    switch (cdsType) {
        ...
        case "DateArray":
            return zod_1.z.array(zDate);
        case "TimeArray":
            return zod_1.z.array(zod_1.z.string());
        case "DateTimeArray":
            return zod_1.z.array(zDateTime);
        case "TimestampArray":
            return zod_1.z.array(zTimestamp);
        case "UUIDArray":
            return zod_1.z.array(zod_1.z.string());
        case "IntegerArray":
        ...
        default:
            return resolveStructuredType(cdsType) ?? zod_1.z.string();
    }
}
/**
 * Resolves a non-builtin operation-parameter type (a custom CDS `type`/struct, optionally `many`)
 * into a Zod schema. parseOperationElements flattens `many <T>` into the string "<T>Array", so we
 * detect the "Array" suffix, look the base type up in the CSN, and:
 *   - for a structured type (has `elements`) build z.object({...}) (element-level optionality from
 *     @mandatory/notNull/key), wrapped in z.array when the param was `many`;
 *   - for a type alias (e.g. `type X : String`) resolve the underlying builtin.
 * Returns undefined when the type can't be resolved, so the caller falls back to z.string().
 * @param cdsType - Flattened CDS type string (e.g. "MyService.IdentifierInputArray")
 * @returns ZodType or undefined
 */
function resolveStructuredType(cdsType) {
    if (typeof cdsType !== "string")
        return undefined;
    const model = cds.model;
    if (!model?.definitions)
        return undefined;
    const isArray = cdsType.endsWith("Array");
    const baseName = isArray ? cdsType.slice(0, -"Array".length) : cdsType;
    const def = model.definitions[baseName];
    if (!def)
        return undefined;
    if (def.elements) {
        const shape = {};
        for (const [elKey, el] of Object.entries(def.elements)) {
            if (!el)
                continue;
            const isMandatory = el["@mandatory"] === true || el.notNull === true || el.key === true;
            // Nested arrayed element (`many X` inside a struct): CSN carries `items`, no `type`.
            // Resolve the item type recursively so e.g. OnboardAgreementInput.specifiers
            // (many OnboardSpecifierInput) survives into the tool schema instead of being
            // silently stripped by zod's default strip mode.
            if (el.items) {
                const itemTypeName = typeof el.items.type === "string" ? el.items.type.replace("cds.", "") : undefined;
                const itemType = itemTypeName
                    ? (resolveStructuredType(itemTypeName) ?? determineMcpParameterType(itemTypeName))
                    : undefined;
                if (itemType) {
                    shape[elKey] = isMandatory ? zod_1.z.array(itemType) : zod_1.z.array(itemType).optional();
                }
                continue;
            }
            ...
            if (el.type === "cds.Association" || el.type === "cds.Composition")
                continue;
            let elZod = determineMcpParameterType(el.type.replace("cds.", ""));
            if (!isMandatory)
                elZod = elZod.optional();
            shape[elKey] = elZod;
        }
        const obj = zod_1.z.object(shape);
        return isArray ? zod_1.z.array(obj) : obj;
    }
    if (typeof def.type === "string") {
        return determineMcpParameterType(def.type.replace("cds.", "") + (isArray ? "Array" : ""));
    }
    return undefined;
}
/**
...
function buildCompositionZodType(key, target) {
    ...
    // CAP's draft runtime manages these columns on the composition's draft
    // sibling (`.texts`, `.items`, …) exactly like on the root. Surfacing them
    // in the composition sub-schema invites the LLM to supply bookkeeping
    // values the user never asked for — same reasoning as DRAFT_INTERNAL_FIELDS
    // on the root's writable schemas.
    const DRAFT_INTERNAL = new Set([
        "IsActiveEntity", "HasActiveEntity", "HasDraftEntity",
        "SiblingEntity", "SiblingEntity_ID",
        "DraftAdministrativeData", "DraftAdministrativeData_DraftUUID",
    ]);
    for (const [k, v] of Object.entries(comp.elements)) {
        if (!v.type)
            continue;
        if (DRAFT_INTERNAL.has(k))
            continue;
        ...
        // Keys that CAP auto-fills (UUIDs via @cds.on.insert, or draft-only
        // surrogate like `ID_texts`) must be optional in the input schema —
        // requiring them forces the LLM to fabricate a value CAP will then
        // throw away. Tagging via `@cds.on.insert` OR a runtime default OR
        // the `ID_texts` pattern covers the cases we see in practice. UUID
        // keys are also optional: `cuid` aspect doesn't emit `@cds.on.insert`
        // in CSN but CAP's runtime still auto-generates UUIDs for them.
        const hasInsertDefault = elementKeys.has("@cds.on.insert")
            || v.default !== undefined
            || v.virtual === true;
        const isDraftSurrogate = k === "ID_texts";
        const isUuidKey = v.key === true && String(v.type) === "cds.UUID";
        const isOptional = !v.key || hasInsertDefault || isDraftSurrogate || isUuidKey;
 ...
 *
 * Side-effect: logs the error at `error` level via the `mcp` channel so
 * every error shown to the client also lands in server logs. Any callers
 * that previously logged explicitly (e.g. `LOGGER.error(msg, error)` right
 * before calling toolError) were doing so to get the raw stack trace —
 * pass that stack through as the optional `cause` arg and this helper
 * handles both lines in one place, avoiding double-logging.
 */
function toolError(code, message, extra, cause) {
    const payload = { error: code, message, ...(extra || {}) };
    try {
        const { LOGGER } = require("../logger");
        if (cause !== undefined) {
            LOGGER.error(`[mcp:error-to-client] ${code}: ${message}`, cause);
        } else {
            LOGGER.error(`[mcp:error-to-client] ${code}: ${message}`);
        }
    }
    catch { }
    return {
...
function asMcpResult(payload) {
    ...
    if (Array.isArray(payload)) {
        // Emit a single JSON-array text part rather than one content item per
        // element. The SDK concatenates multi-part text results with `\n` at
        // the agent boundary, which turns array payloads into
        // `{...}\n{...}\n...` — not valid JSON, so the agent cannot parse it.
        // A single stringified array round-trips cleanly through JSON.parse.
        return {
            content: [
                {
                    type: "text",
