...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .fbtn { display: inline-flex; align-items: center; gap: 0.25rem; height: 2.25rem; padding: 0 0.75rem; border-radius: var(--sapField_BorderCornerRadius); font-size: var(--sapFontSmallSize); font-weight: 600; font-family: inherit; cursor: pointer; border: 0.0625rem solid var(--sapLinkColor); background: transparent; color: var(--sapLinkColor); }
  ...
  .form-row { padding: 0.5rem 0; }
  .form-row label { display: block; font-size: var(--sapFontSmallSize); color: var(--sapContent_LabelColor); margin-bottom: 0.25rem; }
  ...
  .form-row input[type=text], .form-row input[type=number], .form-row input[type=date], .form-row input[type=datetime-local] {
    width: 100%; height: 2.25rem; padding: 0 0.625rem; font-size: var(--sapFontSize); font-family: inherit;
 ...
 * @returns {string}
 */
function buildQueryTemplate(entityName, uiMeta, appTitle, writeSpec) {
...
function renderUi5(data){
  ...
  var rows=Array.isArray(data)?data:[data];
  if(!rows.length){ if(info)info.innerHTML=""; el.innerHTML='<div class="loading">No results</div>'; return; }
  var AL=_colAligns(rows);
  if(window.__tbl){ try{ window.__tbl.destroy(); }catch(e){} window.__tbl=null; }
  // Count + Create live in the UI5 header toolbar; clear the page toolbar.
...
<style>
  ...
  .obj-actions { margin-left: auto; display: flex; gap: 0.5rem; align-self: flex-start; }
...
function _edit(){
  if(!WRITE||!WRITE.update||!LAST_ROW)return;
  var row=LAST_ROW;
  ...
  _openForm({ title:"Edit", submitLabel:"Save", fields:keyF.concat(valF),
    onSubmit:function(values){ return window.__callTool(WRITE.update.tool, values).then(function(res){ if(!(res&&res.isError)) _refreshGet(); return res; }); }
  });
}
...
function _renderActions(){
  ...
  if(WRITE&&WRITE.update){ var e=document.createElement("button"); e.className="fbtn fbtn-emph"; e.textContent="Edit"; e.onclick=_edit; a.appendChild(e); }
