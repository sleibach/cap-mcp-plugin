...
function registerApps(server, annotations, config) {
    ...
    for (const [key, entry] of annotations.entries()) {
        ...
        if (uiMeta.lineItems.length > 0 && modes.includes("query")) {
            ...
            _registerAppResource(server, `${entry.target} list`, uri, uiMeta, "query");
        ...
        if (hasDetailContent && modes.includes("get")) {
            ...
            _registerAppResource(server, `${entry.target} detail`, uri, uiMeta, "detail");
 ...
 */
function _registerAppResource(server, name, uri, uiMeta, templateKind) {
    try {
        const entityName = uri.split("/").pop() ?? name;
        const html =
            templateKind === "query"
                ? template_generator_1.buildQueryTemplate(entityName, uiMeta)
                : template_generator_1.buildDetailTemplate(entityName, uiMeta);

        server.registerResource(name, uri, { mimeType: MCP_APP_MIME_TYPE }, async (_uri) => ({
            contents: [{ uri: uri, mimeType: MCP_APP_MIME_TYPE, text: html }],
        }));
