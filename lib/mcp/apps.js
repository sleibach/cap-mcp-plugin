...
exports._listExposableFields = _listExposableFields;
...
function registerApps(server, annotations, config) {
    ...
    for (const [key, entry] of annotations.entries()) {
        ...
        // Per-entity opt-out: `annotate <Entity> with @mcp.apps: false;`
        if (def["@mcp.apps"] === false) continue;
        ...
        // the entity's write tools back through the host. `apps.write: false`
        // renders read-only apps (no write buttons).
        const writeSpec = config.apps?.write === false ? null : _buildWriteSpec(entry, def, modes, model);
        // `apps.ui5: false` forces the self-contained CSS table (no SAPUI5 CDN load).
        const useUi5 = config.apps?.ui5 !== false;

        if (uiMeta.lineItems.length > 0 && modes.includes("query")) {
            const uri = `ui://query/${entityKey}`;
            const exposableFields = _listExposableFields(entry, def, model, uiMeta);
            _registerAppResource(server, `${entry.target} list`, uri, uiMeta, "query", appTitle, writeSpec, useUi5, exposableFields);
        ...
        if (hasDetailContent && modes.includes("get")) {
            ...
            _registerAppResource(server, `${entry.target} detail`, uri, uiMeta, "detail", appTitle, writeSpec, useUi5);
...
/** Resolves a display label from element annotations, falling back to humanized name. */
function _fieldLabel(name, el) {
    const raw = el?.["@title"] ?? el?.["@Common.Label"];
    if (typeof raw === "string" && !raw.startsWith("{")) return raw;
    return _humanize(name);
}

/** CAP hierarchy projection columns — not meaningful list columns. */
const _HIERARCHY_FIELDS = new Set([
    "LimitedDescendantCount", "DistanceFromRoot", "DrillState", "LimitedRank",
]);

/**
 * Lists scalar/FK fields exposed by the MCP resource (service projection) that
 * may appear as optional table columns when absent from @UI.LineItem.
 * @UI.Hidden fields are excluded entirely.
 */
function _listExposableFields(entry, def, model, uiMeta) {
    ...
    const add = (path, label) => {
        if (!path || seen.has(path) || hidden.has(path)) return;
        seen.add(path);
        result.push({ path, label: label ?? _humanize(path) });
    };

    for (const [k] of keysMap.entries()) {
        if (_DRAFT_INTERNAL.has(k) || k === "IsActiveEntity") continue;
        add(k, _fieldLabel(k, def?.elements?.[k]));
    }

    for (const [name, cdsType] of props.entries()) {
        if (_DRAFT_INTERNAL.has(name) || _HIERARCHY_FIELDS.has(name) || omitted.has(name) || keysMap.has(name) || computed.has(name)) continue;
        const el = def?.elements ? def.elements[name] : undefined;
        if (el && (el["@cds.on.insert"] !== undefined || el["@cds.on.update"] !== undefined)) continue;
        const lower = String(cdsType).toLowerCase();
        const isComposition = (el && el.type === "cds.Composition") || lower.includes("composition");
        if (isComposition) continue;
        const isAssoc = !!(el && el.target) || lower.includes("association");
        if (name === "localized" || (el && typeof el.target === "string" && el.target.endsWith(".texts"))) continue;
        if (isAssoc) {
            const keyRef = el && Array.isArray(el.keys) && el.keys[0] && Array.isArray(el.keys[0].ref)
                ? el.keys[0].ref[0] : "ID";
            add(`${name}_${keyRef}`, _fieldLabel(name, el));
            continue;
        }
        add(name, _fieldLabel(name, el));
    }
 ...
 */
function _buildWriteSpec(entry, def, modes, model) {
    ...
    // Editable non-key fields: scalars + association FK columns. The element
    // metadata (target / managed flags / type) is the source of truth — the
    // CDS-type string alone misses type-aliased associations like `Currency`.
    const fields = [];
    const seen = new Set();
    for (const [name, cdsType] of props.entries()) {
        if (_DRAFT_INTERNAL.has(name) || omitted.has(name) || keysMap.has(name) || computed.has(name)) continue;
        const el = def && def.elements ? def.elements[name] : undefined;
        // Skip framework-managed fields (createdAt/By, modifiedAt/By, …) — set automatically.
        if (el && (el["@cds.on.insert"] !== undefined || el["@cds.on.update"] !== undefined)) continue;
        const lower = String(cdsType).toLowerCase();
        const isComposition = (el && el.type === "cds.Composition") || lower.includes("composition");
        if (isComposition) continue;
        const isAssoc = !!(el && el.target) || lower.includes("association");
        // Skip localized-text helper associations (…texts) and the `localized` nav.
        if (name === "localized" || (el && typeof el.target === "string" && el.target.endsWith(".texts"))) continue;
        if (isAssoc) {
            // FK column = `<assoc>_<targetKey>` (author_ID, genre_ID, currency_code).
            const keyRef = el && Array.isArray(el.keys) && el.keys[0] && Array.isArray(el.keys[0].ref)
                ? el.keys[0].ref[0] : "ID";
            const fk = `${name}_${keyRef}`;
            if (seen.has(fk)) continue;
            seen.add(fk);
            let fkType = "text";
            try {
                const tgt = el && el.target && model && model.definitions[el.target];
                const kt = tgt && tgt.elements && tgt.elements[keyRef] && tgt.elements[keyRef].type;
                if (kt) fkType = _simpleType(kt);
            } catch (e) { /* fall back to text */ }
            fields.push({ name: fk, label: _humanize(name), type: fkType, fk: true });
            continue;
        }
    ...
    return {
        ...
        // Draft roots edit via edit → patch → activate; draft-upsert is create-only.
        update: modes.includes("update") && !draft ? { tool: nm("update") } : null,
        draftUpdate: modes.includes("update") && draft
            ? { edit: nm("draft-edit"), patch: nm("draft-patch"), activate: nm("draft-activate") }
            : null,
...
function _registerAppResource(server, name, uri, uiMeta, templateKind, appTitle, writeSpec, useUi5, exposableFields) {
    try {
        const entityName = uri.split("/").pop() ?? name;
        const opts = { ui5: useUi5 !== false, exposableFields: exposableFields ?? [] };
        const html =
            templateKind === "query"
                ? template_generator_1.buildQueryTemplate(entityName, uiMeta, appTitle, writeSpec, opts)
                : template_generator_1.buildDetailTemplate(entityName, uiMeta, appTitle, writeSpec, opts);
