...
class IASService {
    ...
    async exchangeCodeForToken(code, redirectUri, code_verifier) {
        try {
            ...
            // For confidential clients, use Basic Auth with client secret
            // For public clients, client_id is sent in the body only
            if (this.isConfidentialClient()) {
                ...
                logger_1.LOGGER.debug("IAS token exchange: Using confidential client authentication (client_secret)");
            } else {
    ...
    async refreshAccessToken(refreshToken) {
        try {
            ...
            if (this.isConfidentialClient()) {
                headers.Authorization = `Basic ${Buffer.from(`${this.credentials?.clientid}:${this.credentials?.clientsecret}`).toString("base64")}`;
                logger_1.LOGGER.debug("IAS token refresh: Using confidential client authentication");
            } else {
