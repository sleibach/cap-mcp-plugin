    ...
    try {
        ...
        if (grant_type === "authorization_code") {
            if (!code || !redirect_uri) {
                logger_1.LOGGER.warn(`[AUTH] /token authorization_code missing required params: has_code=${!!code}, has_redirect_uri=${!!redirect_uri}`);
                res.status(400).json({
            ...
            if (!refresh_token) {
                logger_1.LOGGER.warn(`[AUTH] /token refresh_token grant missing refresh_token parameter`);
                res.status(400).json({
                    error: "invalid_request",
            ...
            logger_1.LOGGER.warn(`[AUTH] /token unsupported grant_type=${grant_type}`);
            res.status(400).json({
