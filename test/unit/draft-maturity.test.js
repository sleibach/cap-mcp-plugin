...
describe("Draft lifecycle — enterprise-grade hardening", () => {
  ...
  test("UUID keys: caller-supplied ID round-trips verbatim through draft-new", async () => {
    // CAP's `cuid` aspect doesn't stamp @cds.on.insert in CSN, so our schema
    // builder marks UUID keys as optional. The contract the LLM sees via the
    // describe() hint promises: caller provides → we echo it back; caller
    // omits → CAP generates and we still return it. Verify the "provides" leg.
    const ID = "ffffffff-ffff-4fff-ffff-fffffffffff1";
    const res = await call(
      "orders_draft-new",
      { ID, title: "caller-supplied", priority: "high" },
      "alice",
    );
  ...
  test("UUID keys: omitted ID is generated by CAP and returned", async () => {
    // Verify the "omits" leg — the response must contain the generated ID
    // so the caller can address the draft in draft-patch / draft-activate.
    const res = await call(
      "orders_draft-new",
      { title: "auto-keyed", priority: "low" },
      "alice",
    );
    const row = payload(res);
    ...
    expect(row.IsActiveEntity).toBe(false);
    await call("orders_draft-discard", { ID: row.ID }, "alice");
  });

  test("UUID key input schema advertises optional-with-server-generation semantics", () => {
    // Surface-level regression guard: the `describe` string for `ID` must
    // tell the LLM that CAP generates the UUID when omitted. If this wording
    // drifts, LLMs will start supplying garbage keys or assume required.
    const meta = capturedMeta.get("orders_draft-new");
    expect(meta).toBeDefined();
    const idZod = meta.inputSchema.ID;
    expect(idZod).toBeDefined();
    // Zod stores the description string on `._def.description`.
    const desc = idZod._def?.description ?? "";
    expect(desc).toMatch(/UUID/);
    expect(desc).toMatch(/optional/i);
    expect(desc).toMatch(/generate/i);
  });
});
